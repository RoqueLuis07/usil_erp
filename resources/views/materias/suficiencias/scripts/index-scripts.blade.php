<script type="module">
    ...
    function fetch() {
        $.ajax({
            type: 'GET',
            url: '{{route('materias_suficiencias.index_ajax')}}',
                ...
                $.each(response.materias, function (index, value) {
                    if (value.estado == 'AC') {
                        ...
                        var botones = `<button class="btn btn-sm btn-primary" data-bs-toggle="modal" data-bs-target="#showModal" id="show-btn" data-id="${value.id}" data-bs-toggle="tooltip" data-bs-trigger="hover" data-bs-placement="top" title="Ver"><i class="ri-eye-fill"></i></button>
                                        @can('editar_materias_suficiencia')
                                        ...
                                        @endcan
                                        @can('inactivar_materias_suficiencia')
                                        ...
                                        @endcan
                                        @can('eliminar_materias_suficiencia')
                        ...
                        var botones = `<button class="btn btn-sm btn-primary" data-bs-toggle="modal" data-bs-target="#showModal" id="show-btn" data-id="${value.id}" data-bs-toggle="tooltip" data-bs-trigger="hover" data-bs-placement="top" title="Ver"><i class="ri-eye-fill"></i></button>
                                        ...
                                        @endcan
                                        @can('activar_materias_suficiencia')
                    ...
                    $('tbody').append(
                        `<tr>
                            ...
                            <td class="materia">${value.materia.nombre_real}</td>
    ...
    $('#save-btn').click(function () {
        ...
        swalWithBootstrapButtons.fire({
            title: '¿Está seguro de guardar la nueva materia de suficiencia?',
            icon: 'question',
    ...
    $(document).on('click', '#show-btn', function() {
        ...
        var url = "{{route('materias_suficiencias.show', ":id")}}"
        ...
        $.ajax({
            ...
            $('#nombre-show').val(response.materia.materia.nombre_real);
    ...
    $(document).on('click', '#edit-btn', function() {
        ...
        const id = $(this).data('id');
        var url = "{{route('materias_suficiencias.edit', ":id")}}"
        url = url.replace(':id', id);
        $.ajax({
            type: 'GET',
            url: url,
            ...
            $('#nombre-edit').selectpicker('destroy');
            $('#nombre-edit').empty();
            $('#nombre-edit').append('<option value="" disabled>Seleccionar...</option>');
            $.each(response.materias, function (index, value) {
                if (value.id == response.materia.materia_id) {
                    $('#nombre-edit').append('<option value"' + value.id + '" selected>' + value.nombre_real + '</option>')
                } else {
                    $('#nombre-edit').append('<option value"' + value.id + '">' + value.nombre_real + '</option>')
                }
            })
            $('#nombre-edit').selectpicker('render');
    ...
    $(document).on('click', '#unactivate-btn', function() {
        const id = $(this).data('id');
        var url = "{{route('materias_suficiencias.get_unactivate', ":id")}}"
        url = url.replace(':id', id);
        $.ajax({
            type: 'GET',
            url: url,
            ...
            $('.mensaje').html('Está seguro de inactivar la materia de suficiencia ' + response.materia.materia.nombre_real + '?');
    ...
    $(document).on('click', '#activate-btn', function() {
        const id = $(this).data('id');
        var url = "{{route('materias_suficiencias.get_activate', ":id")}}"
        url = url.replace(':id', id);
        $.ajax({
            type: 'GET',
            url: url,
            ...
            $('.mensaje').html('Está seguro de activar la materia de suficiencia ' + response.materia.materia.nombre_real + '?');
    ...
    $(document).on('click', '#destroy-btn', function() {
        const id = $(this).data('id');
        var url = "{{route('materias_suficiencias.get_destroy', ":id")}}"
        url = url.replace(':id', id);
        $.ajax({
            type: 'GET',
            url: url,
            ...
            $('.mensaje').html('Está seguro de eliminar la materia de suficiencia ' + response.materia.materia.nombre_real + '?');
    ...
    $('.update-btn').click(function () {
        ...
        swalWithBootstrapButtons.fire({
            title: '¿Está seguro de actualizar la materia de suficiencia ' + nombre + '?',
            icon: 'question',
            showCancelButton: true,
    ...
    $('.delete-btn').click(function (e) {
        ...
        var url = "{{route('materias_suficiencias.destroy', ":id")}}"
        url = url.replace(':id', id);
        var type = 'error';
        $.ajax({
            url: url,
    ...
    $('.unactivate-btn').click(function (e) {
        ...
        var url = "{{route('materias_suficiencias.unactivate', ":id")}}"
        url = url.replace(':id', id);
        var type = 'success';
        $.ajax({
            url: url,
    ...
    $('.activate-btn').click(function (e) {
        ...
        var url = "{{route('materias_suficiencias.activate', ":id")}}"
        url = url.replace(':id', id);
        var type = 'success';
        $.ajax({
            url: url,
    ...
    function save() {
        ...
        var type = 'success';
        $.ajax({
            url: '{{route('materias_suficiencias.store')}}',
            data: formData,
            type: 'POST',
            headers: {
                'X-CSRF-TOKEN': '{{csrf_token()}}'
    ...
    function update(id) {
        ...
        var url = "{{route('materias_suficiencias.update', ":id")}}"
        url = url.replace(':id', id);
        var type = 'success';
        $('#update-form').find('.is-invalid').removeClass('is-invalid');
        $('#update-form').find('.invalid-feedback').remove();
