<script type="module">
    ...
    function fetch() {
        $.ajax({
            type: 'GET',
            url: '{{route('cajas.index_ajax')}}',
            success: function (response) {
                $('tbody').html('');
                $.each(response.cajas, function (index, value) {
                    if (value.estado == 'AC') {
                    ...
                    var efectivo = Intl.NumberFormat('de-DE').format(parseInt(value.monto));
                    var cheque = Intl.NumberFormat('de-DE').format(parseInt(value.monto_cheque));
                    $('tbody').append(
                        `<tr>
                            <td class="id">${value.id}</td>
                            <td class="nombre">${value.nombre}</td>
                            <td class="usuario">${value.usuario.name}</td>
                            <td class="efectivo">${efectivo}</td>
                ...
                var options = {
                    valueNames: ['nombre', 'usuario', 'efectivo', 'cheque'],
                    page: 50,
                    pagination: true
                };
                var cajasList = new List('cajas-list', options);
                cajasList.on('updated', function(list) {
    ...
    $('#save-btn').click(function () {
        ...
        swalWithBootstrapButtons.fire({
            title: '¿Está seguro de guardar la nueva caja?',
            icon: 'question',
    ...
    $(document).on('click', '#show-btn', function() {
        ...
        $.ajax({
            ...
            $('#nombre-show').val(response.caja.nombre);
            $('#usuario-show').val(response.caja.usuario.name);
            $('#efectivo-show').val(Intl.NumberFormat('de-DE').format(parseInt(response.caja.monto)));
            $('#cheque-show').val(Intl.NumberFormat('de-DE').format(parseInt(response.caja.monto_cheque)));
            $('#cuenta_ingreso-show').val(response.caja.cuenta_ingreso.nombre + ' - ' + response.caja.cuenta_ingreso.cuenta);
            $('#cuenta_egreso-show').val(response.caja.cuenta_egreso.nombre + ' - ' + response.caja.cuenta_egreso.cuenta);
            if (response.caja.estado == 'AC') {
                $('#estado-show').val('ACTIVO');
            } else if (response.caja.estado == 'IN') {
    ...
    $(document).on('click', '#edit-btn', function() {
        ...
        $.ajax({
            ...
            $('#nombre-edit').val(response.caja.nombre);
            $('#usuario-edit').selectpicker('destroy');
            $('#usuario-edit').empty();
            $('#usuario-edit').append('<option value="" disabled>Seleccionar...</option>');
            $.each(response.usuarios, function (index, value) {
                if (value.id == response.caja.usuario.id) {
                    $('#usuario-edit').append('<option value="' + value.id + '" selected>' + value.name + '</option>');
                } else {
                    $('#usuario-edit').append('<option value="' + value.id + '">' + value.name + '</option>');
                }
            });
            $('#usuario-edit').selectpicker('render');
            ...
            $.each(response.cuentas_contables, function (index, value) {
                if (value.id == response.caja.cuenta_ingreso_id) {
            ...
            $.each(response.cuentas_contables, function (index, value) {
                if (value.id == response.caja.cuenta_egreso_id) {
            ...
            $('#efectivo-edit').val(Intl.NumberFormat('de-DE').format(parseInt(response.caja.monto)));
            $('#cheque-edit').val(Intl.NumberFormat('de-DE').format(parseInt(response.caja.monto_cheque)));
            $('.update-btn').val(response.caja.id);
        }).fail(function(response) {
    ...
    $(document).on('click', '#unactivate-btn', function() {
        const id = $(this).data('id');
        var url = "{{route('cajas.get_unactivate', ":id")}}"
        url = url.replace(':id', id);
        $.ajax({
            type: 'GET',
            url: url,
            ...
            $('.mensaje').html('Está seguro de inactivar la caja ' + response.caja.nombre + '?');
            $('.unactivate-btn').val(response.caja.id)
        }).fail(function(response) {
    ...
    $(document).on('click', '#activate-btn', function() {
        const id = $(this).data('id');
        var url = "{{route('cajas.get_activate', ":id")}}"
        url = url.replace(':id', id);
        $.ajax({
            type: 'GET',
            url: url,
            ...
            $('.mensaje').html('Está seguro de activar la caja ' + response.caja.nombre + '?');
            $('.activate-btn').val(response.caja.id)
        }).fail(function(response) {
    ...
    $(document).on('click', '#destroy-btn', function() {
        const id = $(this).data('id');
        var url = "{{route('cajas.get_destroy', ":id")}}"
        url = url.replace(':id', id);
        $.ajax({
            type: 'GET',
            url: url,
            ...
            $('.mensaje').html('Está seguro de eliminar la caja ' + response.caja.nombre + '?');
            $('.delete-btn').val(response.caja.id)
        }).fail(function(response) {
    ...
    $('.update-btn').click(function () {
        ...
        swalWithBootstrapButtons.fire({
            title: '¿Está seguro de actualizar la caja ' + nombre + '?',
            icon: 'question',
            showCancelButton: true,
    ...
    $('.delete-btn').click(function (e) {
        ...
        var url = "{{route('cajas.destroy', ":id")}}"
        url = url.replace(':id', id);
        var type = 'error';
        $.ajax({
            url: url,
    ...
    $('.unactivate-btn').click(function (e) {
        ...
        var url = "{{route('cajas.unactivate', ":id")}}"
        url = url.replace(':id', id);
        var type = 'success';
        $.ajax({
            url: url,
    ...
    $('.activate-btn').click(function (e) {
        ...
        var url = "{{route('cajas.activate', ":id")}}"
        url = url.replace(':id', id);
        var type = 'success';
        $.ajax({
            url: url,
