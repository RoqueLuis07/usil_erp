...
@section('pdf_content')
    ...
    <div class="tabla">
        ...
        <table class="tabla-compra">
            <thead>
                <tr>
                    <th width="25%" style="text-align: left!important"> &nbsp;&nbsp;&nbsp; Proveedor</th>
                    <th width="20%">Comprobante</th>
                    <th width="20%">Número</th>
                    <th width="15%">Timbrado</th>
            ...
            <tbody>
                <tr>
                    <td style="text-align: left!important">{{ $orden_pago->compra->proveedor->razon_social }}</td>
                    <td>FACTURA</td>
                    <td>{{ $orden_pago->compra->numero_factura }}</td>
                    <td>{{ $orden_pago->compra->timbradoProveedor->numero }}</td>
                    <td>{{ Carbon\Carbon::parse($orden_pago->compra->fecha)->format('d/m/Y') }}</td>
                    <td>{{ number_format($orden_pago->compra->monto_total, $decimales, ',', '.') }}</td>
                </tr>
                <tr>
                    <td style="text-align: right!important;">{{ $orden_pago->compra->unidad_negocio_id }} <span style="color:gray;">{{ $orden_pago->compra->unidadNegocio->nombre }}</span></td>
                    <td style="color: gray;" colspan="4">{{ $orden_pago->compra->subunidad_negocio_id }} {{ $orden_pago->compra->subunidadNegocio->nombre }}</td>
                    <td style="text-align: left!important; color: gray;">{{ number_format($orden_pago->compra->monto_total, $decimales, ',', '.') }}</td>
                </tr>
            </tbody>
        </table>
    </div>
    ...
    @if ($orden_pago->asientoContable && $orden_pago->asientoContable->estado == 'AC')
        <div class="tabla-asiento">
            <table class="tabla-asiento">
                ...
                <tbody>
                    @foreach ($orden_pago->asientoContable->detalles as $detalle)
                        <tr>
                            <td colspan="2">{{ $detalle->cuentaContable->cuenta }}</td>
                            <td colspan="4">{{ $detalle->cuentaContable->nombre }}
                                <br>
                                <span style="font-size: 8px;">{{ $detalle->descripcion }}</span>
                                <br>
                                <span style="font-size: 8px;">CC: &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; {{ $detalle->centroCosto->id }} {{ $detalle->centroCosto->nombre }} &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; SCC: {{ $detalle->subcentroCosto->id }} {{ $detalle->subcentroCosto->nombre }}</span>
                            </td>
