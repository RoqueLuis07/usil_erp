<script type="module">
    ...
    function fetch() {
        $.ajax({
            type: 'GET',
            url: '{{route('alumnos_formaciones.index_ajax')}}',
            success: function (response) {
                $('tbody').html('');
                $.each(response.alumnos_formaciones, function (index, value) {
                    if (value.estado == 'AC') {
                        ...
                        var botones = `<button class="btn btn-sm btn-primary" data-bs-toggle="modal" data-bs-target="#showModal" id="show-btn" data-id="${value.id}" data-bs-toggle="tooltip" data-bs-trigger="hover" data-bs-placement="top" title="Ver"><i class="ri-eye-fill"></i></button>
                                        @can('editar_formaciones_academicas')
                                        ...
                                        @endcan
                                        @can('inactivar_formaciones_academicas')
                                        ...
                                        @endcan
                                        @can('eliminar_formaciones_academicas')
                        ...
                        var botones = `<button class="btn btn-sm btn-primary" data-bs-toggle="modal" data-bs-target="#showModal" id="show-btn" data-id="${value.id}" data-bs-toggle="tooltip" data-bs-trigger="hover" data-bs-placement="top" title="Ver"><i class="ri-eye-fill"></i></button>
                                        ...
                                        @endcan
                                        @can('activar_formaciones_academicas')
                ...
                var alumnos_formacionesList = new List('alumnos_formaciones-list', options);
                alumnos_formacionesList.on('updated', function(list) {
    ...
    $('#save-btn').click(function () {
        ...
        swalWithBootstrapButtons.fire({
            title: '¿Está seguro de guardar la nueva formación académica?',
            icon: 'question',
    ...
    $(document).on('click', '#show-btn', function() {
        ...
        var url = "{{route('alumnos_formaciones.show', ":id")}}"
        ...
        $.ajax({
            ...
            $('#nombre-show').val(response.alumno_formacion.nombre);
            if (response.alumno_formacion.estado == 'AC') {
                $('#estado-show').val('ACTIVO');
            } else if (response.alumno_formacion.estado == 'IN') {
    ...
    $(document).on('click', '#edit-btn', function() {
        ...
        const id = $(this).data('id');
        var url = "{{route('alumnos_formaciones.edit', ":id")}}"
        url = url.replace(':id', id);
        $.ajax({
            type: 'GET',
            url: url,
            ...
            $('#nombre-formacion-academica-edit').val(response.alumno_formacion.nombre);
            $('.update-btn').val(response.alumno_formacion.id);
        })
    });

    $(document).on('click', '#unactivate-btn', function() {
        const id = $(this).data('id');
        var url = "{{route('alumnos_formaciones.get_unactivate', ":id")}}"
        url = url.replace(':id', id);
        $.ajax({
            type: 'GET',
            url: url,
            ...
            $('.mensaje').html('Está seguro de inactivar la formación académica ' + response.alumno_formacion.nombre + '?');
            $('.unactivate-btn').val(response.alumno_formacion.id)
        }).fail(function(response) {
    ...
    $(document).on('click', '#activate-btn', function() {
        const id = $(this).data('id');
        var url = "{{route('alumnos_formaciones.get_activate', ":id")}}"
        url = url.replace(':id', id);
        $.ajax({
            type: 'GET',
            url: url,
            ...
            $('.mensaje').html('Está seguro de activar la formación académica ' + response.alumno_formacion.nombre + '?');
            $('.activate-btn').val(response.alumno_formacion.id)
        }).fail(function(response) {
    ...
    $(document).on('click', '#destroy-btn', function() {
        const id = $(this).data('id');
        var url = "{{route('alumnos_formaciones.get_destroy', ":id")}}"
        url = url.replace(':id', id);
        $.ajax({
            type: 'GET',
            url: url,
            ...
            $('.mensaje').html('Está seguro de eliminar la formación académica ' + response.alumno_formacion.nombre + '?');
            $('.delete-btn').val(response.alumno_formacion.id)
        }).fail(function(response) {
    ...
    $('.update-btn').click(function () {
        var nombre = $('#nombre-formacion-academica-edit').val();
        const swalWithBootstrapButtons = Swal.mixin({
            customClass: {
                confirmButton: 'btn btn-success me-2',
                cancelButton: 'btn btn-light',
        ...
        swalWithBootstrapButtons.fire({
            title: '¿Está seguro de actualizar la formación académica ' + nombre + '?',
            icon: 'question',
            showCancelButton: true,
    ...
    $('.delete-btn').click(function (e) {
        ...
        var url = "{{route('alumnos_formaciones.destroy', ":id")}}"
        url = url.replace(':id', id);
        var type = 'error';
        $.ajax({
            url: url,
    ...
    $('.unactivate-btn').click(function (e) {
        ...
        var url = "{{route('alumnos_formaciones.unactivate', ":id")}}"
        url = url.replace(':id', id);
        var type = 'success';
        $.ajax({
            url: url,
    ...
    $('.activate-btn').click(function (e) {
        ...
        var url = "{{route('alumnos_formaciones.activate', ":id")}}"
        url = url.replace(':id', id);
        var type = 'success';
        $.ajax({
            url: url,
    ...
    function save() {
        ...
        var type = 'success';
        $.ajax({
            url: '{{route('alumnos_formaciones.store')}}',
            data: formData,
            type: 'POST',
            headers: {
                'X-CSRF-TOKEN': '{{csrf_token()}}'
    ...
    function update(id) {
        ...
        var url = "{{route('alumnos_formaciones.update', ":id")}}"
        url = url.replace(':id', id);
        var type = 'success';
        $('#update-form').find('.is-invalid').removeClass('is-invalid');
        $('#update-form').find('.invalid-feedback').remove();
