@foreach ($unidad_negocio->subunidadesNegociosContables as $detalle)
    <!-- changeEstado -->
        @can('inactivar_subunidades_negocios_contables')
                ...
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content">
                        <div class="modal-body">
                            ...
                            <form id="changeEstado-form-{{$detalle->id}}" action="{{route('subunidades_negocios_contables.change_estado', $detalle->id)}}" method="post">
                                @csrf
                                <div class="row">
                                    <div class="col-lg-12 mb-3 text-center">
                                        @if ($detalle->estado == 'AC')
                                            <h4>¿Está seguro de inactivar la subunidad de negocio {{$detalle->nombre}}?</h4>
                                        @elseif ($detalle->estado == 'IN')
                                            <h4>¿Está seguro de activar la subunidad de negocio {{$detalle->nombre}}?</h4>
                                        @endif
                                    </div>
                                </div>
                                <div class="row">
