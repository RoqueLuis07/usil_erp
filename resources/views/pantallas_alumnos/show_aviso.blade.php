@can('ver_noticias_avisos_alumnos_pantalla')
    ...
    @section('title') Ver Aviso @endsection
    @section('content')
                            ...
                            <div class="card overflow-hidden">
                                <div class="bg-primary pb-0">
                                    <div class="card-body text-center py-4">
                                        <h4 class="fw-medium text-white mb-3">{{Str::title($aviso->titulo)}}</h4>
                                    </div>
                                    <div class="card-body py-2 bg-white bg-opacity-10">
                                        <ul class="list-unstyled mb-0 text-white-75 hstack gap-2 justify-content-between">
                                            <li><b>Creado por:</b> {{Str::title($aviso->cargadoPor->name)}}</li>
                                            <li>{{Carbon\Carbon::parse($aviso->fecha_hora_publicacion)->translatedFormat('j \d\e F \d\e\l Y')}}</li>
                                        </ul>
                                    </div>
                                </div>
                                <div class="card-body text-muted">
                                    <p>{!! $aviso->descripcion !!}</p>
                                    <p class="text-center"><img src="{{asset($aviso->portada)}}" alt="Portada" style="width: 300px;"></p>
                                </div>
                            </div>
                        </div>
                    </div>
