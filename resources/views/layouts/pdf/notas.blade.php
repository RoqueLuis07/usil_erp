...
<body>
    ...
    <footer>
        <div id="footer-text">
            <span><b>Generado el </b>{{Str::lower($fecha_hoy->locale('es')->translatedFormat('l'))}} {{$fecha_hoy->format('d/m/Y')}} <b>a las</b> {{$fecha_hoy->format('H:i:s')}} <b>@if (!Auth::user()->hasRole('ALUMNO'))por el usuario</b> {{Str::title(Auth::user()->name)}}@endif</span>
        </div>
