<script type="module">
    ...
    function fetch() {
        $.ajax({
            type: 'GET',
            url: '{{route('modalidades.index_ajax')}}',
            success: function (response) {
                $('tbody').html('');
                $.each(response.modalidades, function (index, value) {
                    if (value.estado == 'AC') {
                ...
                var modalidadesList = new List('modalidades-list', options);
                modalidadesList.on('updated', function(list) {
    ...
    $('#save-btn').click(function () {
        ...
        swalWithBootstrapButtons.fire({
            title: '¿Está seguro de guardar la nueva modalidad?',
            icon: 'question',
    ...
    $(document).on('click', '#show-btn', function() {
        ...
        var url = "{{route('modalidades.show', ":id")}}"
        ...
        $.ajax({
            ...
            $('#nombre-show').val(response.modalidad.nombre);
            if (response.modalidad.estado == 'AC') {
                $('#estado-show').val('ACTIVO');
            } else if (response.modalidad.estado == 'IN') {
    ...
    $(document).on('click', '#edit-btn', function() {
        ...
        const id = $(this).data('id');
        var url = "{{route('modalidades.edit', ":id")}}"
        url = url.replace(':id', id);
        $.ajax({
            type: 'GET',
            url: url,
            ...
            $('#nombre-edit').val(response.modalidad.nombre);
            $('.update-btn').val(response.modalidad.id);
        }).fail(function(response) {
    ...
    $(document).on('click', '#unactivate-btn', function() {
        const id = $(this).data('id');
        var url = "{{route('modalidades.get_unactivate', ":id")}}"
        url = url.replace(':id', id);
        $.ajax({
            type: 'GET',
            url: url,
            ...
            $('.mensaje').html('Está seguro de inactivar la modalidad ' + response.modalidad.nombre + '?');
            $('.unactivate-btn').val(response.modalidad.id)
        }).fail(function(response) {
    ...
    $(document).on('click', '#activate-btn', function() {
        const id = $(this).data('id');
        var url = "{{route('modalidades.get_activate', ":id")}}"
        url = url.replace(':id', id);
        $.ajax({
            type: 'GET',
            url: url,
            ...
            $('.mensaje').html('Está seguro de activar la modalidad ' + response.modalidad.nombre + '?');
            $('.activate-btn').val(response.modalidad.id)
        }).fail(function(response) {
    ...
    $(document).on('click', '#destroy-btn', function() {
        const id = $(this).data('id');
        var url = "{{route('modalidades.get_destroy', ":id")}}"
        url = url.replace(':id', id);
        $.ajax({
            type: 'GET',
            url: url,
            ...
            $('.mensaje').html('Está seguro de eliminar la modalidad ' + response.modalidad.nombre + '?');
            $('.delete-btn').val(response.modalidad.id)
        }).fail(function(response) {
    ...
    $('.update-btn').click(function () {
        ...
        swalWithBootstrapButtons.fire({
            title: '¿Está seguro de actualizar la modalidad ' + nombre + '?',
            icon: 'question',
            showCancelButton: true,
    ...
    $('.delete-btn').click(function (e) {
        ...
        var url = "{{route('modalidades.destroy', ":id")}}"
        url = url.replace(':id', id);
        var type = 'error';
        $.ajax({
            url: url,
    ...
    $('.unactivate-btn').click(function (e) {
        ...
        var url = "{{route('modalidades.unactivate', ":id")}}"
        url = url.replace(':id', id);
        var type = 'success';
        $.ajax({
            url: url,
    ...
    $('.activate-btn').click(function (e) {
        ...
        var url = "{{route('modalidades.activate', ":id")}}"
        url = url.replace(':id', id);
        var type = 'success';
        $.ajax({
            url: url,
    ...
    function update(id) {
        ...
        var url = "{{route('modalidades.update', ":id")}}"
        url = url.replace(':id', id);
        var type = 'success';
        $('#update-form').find('.is-invalid').removeClass('is-invalid');
        $('#update-form').find('.invalid-feedback').remove();
