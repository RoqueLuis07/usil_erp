                ...
                <div class="col-lg-12">
                        ...
                        <div class="card-body">
							...
							<div class="row">
                                <div class="col-lg-2 mb-3 text-center">
                                    <label class="form-label" for="total_clases">Clases Desarrolladas</label>
                                    <input type="text" class="form-control text-center" id="total_clases" value="{{$materia->total_clases}}" readonly>
                                </div>
								<div class="col-lg-2 mb-3 text-center">
                                    <label class="form-label" for="total_horas">Horas Desarrolladas</label>
                                    <input type="text" class="form-control text-center" id="total_horas" value="{{$materia->horas_desarrollo}}" readonly>
                                </div>
                            </div>
                        </div>
                    </div>
                                        ...
                                        <div class="table-responsive table-card mt-3 mb-1">
                                            <table class="table align-middle table-nowrap text-center" id="asistencias-list">
                                                ...
                                                <tbody class="list form-check-all">
                                                    @forelse ($asistencias as $alumno_id => $asistencias_alumno)
														@php
															$alumno = $asistencias_alumno->first()->alumno; // Obtener el primer alumno
														@endphp
														<tr>
															<td>{{ $alumno->primer_nombre }} {{ $alumno->primer_apellido }}</td>
															<td>{{ $alumno->numero_documento }}</td>
															<td>{{ $materia->total_clases }}</td>
															<td>{{ $asistencias_alumno->first()->horas_asistidas }}</td>
															<td>{{ $asistencias_alumno->first()->porcentaje }}%</td>
														</tr>
													@empty
														<tr>
															<td colspan="5">La materia no cuenta con asistencias cargadas.</td>
														</tr>
													@endforelse
                                                </tbody>
                                            </table>
                    ...
                    <div class="row">
                        <div class="col-lg-12 text-end mb-3">
                            <a type="button" class="btn btn-danger me-2" href="{{route('pantallas_docentes.asistencias_alumnos', Auth::id())}}">Volver</a>
                        </div>
                    </div>
                </div>
            </form>
