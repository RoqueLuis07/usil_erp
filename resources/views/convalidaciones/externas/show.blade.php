@can('ver_convalidaciones_externas')
    ...
    @section('title') Ver Convalidación Externa @endsection
    @section('css')
    ...
    @section('content')
        @component('components.breadcrumb')
            ...
            @slot('title') Ver Convalidación Externa  @endslot
        @endcomponent

        @include('convalidaciones.externas.scripts.messages-scripts')
        @include('convalidaciones.externas.modals.show-modals')
                ...
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-header d-flex flex-wrap">
                            <div class="col-lg-6">
                                <h4 class="card-title mb-0">Visualizar convalidación externa</h4>
                            </div>
                        </div>
                        <!-- end card header -->
                        <div class="card-body">
                            ...
                            <div class="row">
                                ...
                                <div class="col-lg-2 mb-3 text-center">
                                    <label class="form-label" for="certificado_estudios">Certificado de Estudios</label>
                                    <div class="text-center">
                                        <a type="button" class="btn btn-warning" href="{{asset($convalidacion->ubicacion_certificado_estudios)}}" target="_blank">Ver</a>
                                    </div>
                                </div>
                            </div>
                        </div>
                                                        ...
                                                        @if ($convalidacion->estado != 'CO')
                                                            @if (!$detalle->numero_dictamen)
                                                                @can('dictaminar_convalidaciones_externas')
                                                                    <a type="button" class="btn btn-icon btn-info btn-dictaminar" id="btn-dictaminar-{{$detalle->id}}" data-bs-toggle="modal" data-bs-target="#dictaminarModal-{{$detalle->id}}" data-bs-toggle="tooltip" data-bs-trigger="hover" data-bs-placement="top" title="Dictaminar" data-id="{{$detalle->id}}" data-url="{{route('convalidaciones_externas.get_numero_dictamen', $detalle->id)}}"><i class="ri-edit-fill"></i></a>
                                                                @endcan
                                                            @elseif (!$detalle->numero_resolucion)
                                                                @can('editar_dictamen_convalidaciones_externas')
                                                                    <a type="button" class="btn btn-icon btn-warning btn-dictaminar" id="btn-dictaminar-{{$detalle->id}}" data-bs-toggle="modal" data-bs-target="#dictaminarModal-{{$detalle->id}}" data-bs-toggle="tooltip" data-bs-trigger="hover" data-bs-placement="top" title="Editar Dictamen" data-id="{{$detalle->id}}" data-url="{{route('convalidaciones_externas.get_numero_dictamen', $detalle->id)}}"><i class="ri-edit-fill"></i></a>
                                                                @endcan
                                                                @can('anular_dictamen_convalidaciones_externas')
                                                                ...
                                                                @can('resolucion_convalidaciones_externas')
                                                                    <a type="button" class="btn btn-icon btn-success btn-aprobar" id="btn-aprobar-{{$detalle->id}}" data-bs-toggle="modal" data-bs-target="#aprobarModal-{{$detalle->id}}" data-bs-toggle="tooltip" data-bs-trigger="hover" data-bs-placement="top" title="Aprobar" data-id="{{$detalle->id}}" data-url="{{route('convalidaciones_externas.get_numero_resolucion', $detalle->id)}}"><i class="ri-check-fill"></i></a>
                                                                @endcan
                                                            @elseif ($detalle->numero_resolucion)
                                                                @can('editar_resolucion_convalidaciones_externas')
                                                                    <a type="button" class="btn btn-icon btn-warning btn-aprobar" id="btn-aprobar-{{$detalle->id}}" data-bs-toggle="modal" data-bs-target="#aprobarModal-{{$detalle->id}}" data-bs-toggle="tooltip" data-bs-trigger="hover" data-bs-placement="top" title="Editar Aprobación" data-id="{{$detalle->id}}" data-url="{{route('convalidaciones_externas.get_numero_resolucion', $detalle->id)}}"><i class="ri-check-fill"></i></a>
                                                                @endcan
                                                                @can('anular_resolucion_convalidaciones_externas')
                    ...
                    <div class="row">
                        <div class="col-lg-12 text-end mb-3">
                            <a type="button" class="btn btn-danger me-2" href="{{route('convalidaciones_externas.index')}}">Volver</a>
                        </div>
                    </div>
                </div>
            </form>
            @can('anular_dictamen_convalidaciones_externas')
                <form action="{{route('convalidaciones_externas.cancel_dictamen', $detalle->id)}}" method="post" id="cancel-dictamen-form-{{$detalle->id}}">
                    @csrf
                </form>
            @endcan
            @can('anular_resolucion_convalidaciones_externas')
                <form action="{{route('convalidaciones_externas.cancel_resolucion', $detalle->id)}}" method="post" id="cancel-resolucion-form-{{$detalle->id}}">
                    @csrf
                </form>
            @endcan
        </div>
    ...
    @section('script')
        ...
        @include('convalidaciones.externas.scripts.show-scripts')
    @endsection
@endcan
