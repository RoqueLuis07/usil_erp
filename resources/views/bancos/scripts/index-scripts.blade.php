<script type="module">
    ...
    function fetch() {
        $.ajax({
            ...
            success: function (response) {
                ...
                var bancosList = new List('bancos-list', options);
                bancosList.on('updated', function(list) {
    ...
    $('#save-btn').click(function () {
        ...
        swalWithBootstrapButtons.fire({
            title: '¿Está seguro de guardar el nuevo banco?',
            icon: 'question',
    ...
    $(document).on('click', '#show-btn', function() {
        ...
        $.ajax({
            ...
            $('#nombre-show').val(response.banco.nombre);
            if (response.banco.estado == 'AC') {
                $('#estado-show').val('ACTIVO');
            } else if (response.banco.estado == 'IN') {
    ...
    $(document).on('click', '#edit-btn', function() {
        ...
        $.ajax({
            ...
            $('#nombre-edit').val(response.banco.nombre);
            $('.update-btn').val(response.banco.id);
        }).fail(function(response) {
    ...
    $(document).on('click', '#unactivate-btn', function() {
        const id = $(this).data('id');
        var url = "{{route('bancos.get_unactivate', ":id")}}"
        url = url.replace(':id', id);
        $.ajax({
            type: 'GET',
            url: url,
            ...
            $('.mensaje').html('Está seguro de inactivar el banco ' + response.banco.nombre + '?');
            $('.unactivate-btn').val(response.banco.id)
        }).fail(function(response) {
    ...
    $(document).on('click', '#activate-btn', function() {
        const id = $(this).data('id');
        var url = "{{route('bancos.get_activate', ":id")}}"
        url = url.replace(':id', id);
        $.ajax({
            type: 'GET',
            url: url,
            ...
            $('.mensaje').html('Está seguro de activar el banco ' + response.banco.nombre + '?');
            $('.activate-btn').val(response.banco.id)
        }).fail(function(response) {
    ...
    $(document).on('click', '#destroy-btn', function() {
        const id = $(this).data('id');
        var url = "{{route('bancos.get_destroy', ":id")}}"
        url = url.replace(':id', id);
        $.ajax({
            type: 'GET',
            url: url,
            ...
            $('.mensaje').html('Está seguro de eliminar el banco ' + response.banco.nombre + '?');
            $('.delete-btn').val(response.banco.id)
        }).fail(function(response) {
    ...
    $('.update-btn').click(function () {
        ...
        swalWithBootstrapButtons.fire({
            title: '¿Está seguro de actualizar el banco ' + nombre + '?',
            icon: 'question',
            showCancelButton: true,
    ...
    $('.delete-btn').click(function (e) {
        ...
        var url = "{{route('bancos.destroy', ":id")}}"
        url = url.replace(':id', id);
        var type = 'error';
        $.ajax({
            url: url,
    ...
    $('.unactivate-btn').click(function (e) {
        ...
        var url = "{{route('bancos.unactivate', ":id")}}"
        url = url.replace(':id', id);
        var type = 'success';
        $.ajax({
            url: url,
    ...
    $('.activate-btn').click(function (e) {
        ...
        var url = "{{route('bancos.activate', ":id")}}"
        url = url.replace(':id', id);
        var type = 'success';
        $.ajax({
            url: url,
    ...
    function update(id) {
        ...
        var url = "{{route('bancos.update', ":id")}}"
        url = url.replace(':id', id);
        var type = 'success';
        $('#update-form').find('.is-invalid').removeClass('is-invalid');
        $('#update-form').find('.invalid-feedback').remove();
