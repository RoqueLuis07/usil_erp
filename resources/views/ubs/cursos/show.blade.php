@can('ver_cursos_ubs')
    ...
    @section('title') Ver Curso @endsection
    @section('content')
        @component('components.breadcrumb')
            @slot('li_1') Cursos @endslot
            @slot('title') Ver Curso  @endslot
        @endcomponent
                ...
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-header d-flex flex-wrap">
                            <div class="col-lg-6">
                                <h4 class="card-title mb-0">Visualizar curso</h4>
                            </div>
                            @can('ver_certificados_generados_cursos_ubs')
                                <div class="col-lg-6 text-end">
                                    <a class="btn btn-info" href="{{route('cursos.certificados_generados', $curso->id)}}">Certificados Generados</a>
                                </div>
                            @endcan
                        </div>
                        <!-- end card header -->
                                    ...
                                    <div class="col-lg-9">
                                        <div class="row">
                                            ...
                                            <div class="col-lg-2 mb-3">
                                                ...
                                                <input type="text" class="form-control" id="programa" @if ($curso->programa_id) value="{{$curso->programa->nombre}}" @endif readonly>
                                            </div>
                                            <div class="col-lg-2 mb-3">
                                                <label class="form-label" for="facultad">Facultad</label>
                                                <input type="text" class="form-control" id="facultad" @if ($curso->facultad_id) value="{{$curso->facultad->nombre}}" @endif readonly>
                                            </div>
                                        </div>
                                        <div class="row">
                                            <div class="col-lg-2 mb-3">
                                        ...
                                        <div class="row">
                                            <div class="col-lg-2 mb-3">
                                                ...
                                                <input type="text" class="form-control" id="numero_ley" value="{{$curso->numero_ley}}" readonly>
                                            </div>
                                            <div class="col-lg-2 mb-3">
                                                <label class="form-label" for="numero_acta">N° de Acta</label>
                                                <input type="text" class="form-control" id="numero_acta" value="{{$curso->numero_acta}}" readonly>
                                            </div>
                                            <div class="col-lg-2 mb-3">
                                                <label class="form-label" for="numero_resolucion_cones">N° de Res. del CONES</label>
                                                <input type="text" class="form-control" id="numero_resolucion_cones" value="{{$curso->numero_resolucion_cones}}" readonly>
                                            </div>
                                            <div class="col-lg-2 mb-3 text-center">
                                                ...
                                                <div class="btn-group" role="group">
                                                    <input type="radio" class="btn-check evaluacion1" id="evaluacion1" name="evaluacion" value="false" @if (old('evaluacion') == 'false' || $curso->evaluacion == false) checked @endif disabled>
                                                    <label class="btn btn-outline-danger" for="evaluacion1">No</label>
                                                    <input type="radio" class="btn-check evaluacion2" id="evaluacion2" name="evaluacion" value="true" @if (old('evaluacion') == 'true' || $curso->evaluacion == true) checked @endif disabled>
                                ...
                                <div class="card-body">
                                    <div class="row">
                                        <div class="col-lg-2">
                                            ...
                                            <input type="text" class="form-control text-center" id="precio_contado" value="{{number_format($curso->precios->precio_contado, 0, ',', '.')}}" readonly>
                                        </div>
                                        <div class="col-lg-2">
                                            <label class="form-label" for="cantidad_cuotas">Cant. Cuotas</label>
                                            <input type="text" class="form-control text-center" id="cantidad_cuotas" value="{{number_format($curso->precios->cantidad_cuotas, 0, ',', '.')}}" readonly>
                                        </div>
                                        <div class="col-lg-2">
                                            <label class="form-label" for="precio_cuota">Cuota</label>
                                            <input type="text" class="form-control text-center" id="precio_cuota" value="{{number_format($curso->precios->precio_cuota, 0, ',', '.')}}" readonly>
                                        </div>
                                        <div class="col-lg-2">
                                            <label class="form-label" for="fecha_inicio_vencimiento_cuota">Fecha 1er Venc.</label>
                                            <input type="text" class="form-control text-center" id="fecha_inicio_vencimiento_cuota" value="{{Carbon\Carbon::parse($curso->precios->fecha_inicio_vencimiento_cuota, 0, ',', '.')->format('d/m/Y')}}" readonly>
                                        </div>
                                        <div class="col-lg-2">
                                            <label class="form-label" for="dia_vencimiento_cuota">Día Venc.</label>
                                            <input type="text" class="form-control text-center" id="dia_vencimiento_cuota" value="{{number_format($curso->precios->dia_vencimiento_cuota, 0, ',', '.')}}" readonly>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                            ...
                                            @if ($curso->modulos->count() > 0)
                                                <ol>
                                                    @foreach ($curso->modulos as $detalle)
                                                        <li>{{$detalle->modulo->nombre_fantasia}} - <small class="text-muted">{{$detalle->docente->primer_nombre}} {{$detalle->docente->primer_apellido}}</small></li>
                                                    @endforeach
                                                </ol>
                                            @else
                                                <p>El curso no cuenta con módulos asignados.</p>
                                            @endif
                                        </div>
                                    </div>
                                </div>
                                                            ...
                                                            <div class="col-lg-12 mb-3">
                                                                ...
                                                                @else
                                                                    <div class="col-lg-12 text-center">
                                                                        <h6>El curso no cuenta con alumnos inscriptos actualmente.</h6>
                                                                    </div>
                                                                @endif
                                                            </div>
                                                        </div>
                    ...
                    <div class="row">
                        <div class="col-lg-6 mb-3">
                            ...
                            <br>
                            {{$curso->cargadoPor->name}}, en fecha: {{\Carbon\Carbon::parse($curso->created_at)->format('d/m/Y H:i:s')}}
                        </div>
                        @if ($curso->actualizado_por_id)
                            <div class="col-lg-6 mb-3">
                                <label class="form-label" for="cargado_por">Última actualización hecha por:</label>
                                <br>
                                {{$curso->actualizadoPor->name}}, en fecha: {{\Carbon\Carbon::parse($curso->updated_at)->format('d/m/Y H:i:s')}}
                            </div>
                        @endif
                    </div>
                    <div class="row">
    ...
    @section('script')
        ...
        @include('ubs.cursos.scripts.show-scripts')
    @endsection
@endcan
