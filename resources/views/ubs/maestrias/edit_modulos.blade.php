@can('editar_modulos_en_maestrias_ubs')
    ...
    @section('title') @if ($maestria->modulos->count() > 0) Editar Módulos de la Maestría @else Agregar Módulos a la Maestría @endif @endsection
    @section('css')
    ...
    @section('content')
        @component('components.breadcrumb')
            ...
            @slot('title') @if ($maestria->modulos->count() > 0) Editar Módulos de la Maestría @else Agregar Módulos a la Maestría @endif @endslot
        @endcomponent

        @include('ubs.maestrias.modals.edit_modulos-modals')

        <div class="row">
            <form action="{{route('maestrias.update_modulos', $maestria->id)}}" method="post" id="update-form">
                @csrf
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-header d-flex flex-wrap">
                            <div class="col-lg-6">
                                <h4 class="card-title mb-0">@if ($maestria->modulos->count() > 0) Actualizar módulos del maestria @else Agregar módulos al maestria @endif </h4>
                            </div>
                        </div>
                        <!-- end card header -->
                        <div class="card-body">
                        ...
                        <div class="col-lg-12 mb-3">
                            <div class="card">
                                <div class="card-header">
                                    <h4 class="card-title mb-0">Módulos de la Maestría</h4>
                                    <input type="hidden" name="nuevo" @if ($maestria->modulos->count() > 0) value="NO" @else value="SI" @endif>
                                </div>
                                <div class="card-body">
                                    @forelse ($maestria->modulos as $key => $detalle)
                                        <div class="mb-2 fila" id="fila-{{$key}}">
                                            <div class="row d-flex flex-wrap justify-content-center">
                                                <div class="col-lg-1 mb-2 text-center" id="div-semestre-{{$key}}">
                                                    @if ($key == 0) <label class="form-label label-semestre">Semestre <span class="text-danger">(*)</span></label> @endif
                                                    <input type="text" class="form-control text-center semestre-{{$key}} semestre @error('detalles.'. $key . '.semestre') is-invalid @enderror" id="semestre-{{$key}}" name="detalles[{{$key}}][semestre]" value="{{old('detalles.' . $key . '.semestre', $detalle->semestre)}}" data-id="{{$key}}"></input>
                                                ...
                                                <div class="col-lg-2 mb-2 text-center" id="div-docente-{{$key}}">
                                    ...
                                    @empty
                                        <div class="mb-2 fila" id="fila-0">
                                            <div class="row d-flex flex-wrap justify-content-center">
                                                <div class="col-lg-1 mb-2 text-center" id="div-semestre-0">
                                                    <label class="form-label label-semestre">Semestre <span class="text-danger">(*)</span></label>
                                                    <input type="text" class="form-control text-center semestre-0 semestre @error('detalles.0.semestre') is-invalid @enderror" id="semestre-0" name="detalles[0][semestre]" value="{{old('detalles.0.semestre')}}" data-id="0"></input>
                                                ...
                                                <div class="col-lg-3 mb-2 text-center" id="div-modulo-0">
                                                ...
                                                <div class="col-lg-2 mb-2 text-center" id="div-docente-0">
    ...
    @section('script')
        ...
        @include('ubs.maestrias.scripts.edit_modulos-scripts')
    @endsection
@endcan
