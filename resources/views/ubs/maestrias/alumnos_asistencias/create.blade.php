@can('crear_asistencias_maestrias_ubs')
    ...
    @section('content')
        ...
        <div class="row">
            <form action="{{route('alumnos_asistencias_maestrias.store', ['curso' => $maestria->curso_id, 'modulo' => $maestria->modulo_id, 'docente' => $maestria->docente_id])}}" method="post" id="store-form">
                @csrf
                                ...
                                <div class="col-lg-2 mb-3">
                                    ...
                                    <select class="form-control selectpicker @error('modalidad') is-invalid @enderror" id="modalidad" name="modalidad" data-live-search="true">
                                        ...
                                        @foreach ($modalidades as $modalidad)
                                            <option value="{{$modalidad->id}}" @if (old('modalidad') == strval($modalidad->id) || $maestria->curso->modalidad_id == strval($modalidad->id)) selected @endif>{{$modalidad->nombre}}</option>
                                        @endforeach
                                    </select>
                                    @error('fecha')
                                        <span class="invalid-feedback" role="alert">
    ...
    @section('script')
        ...
        @include('ubs.maestrias.alumnos_asistencias.scripts.create-scripts')
    @endsection
@endcan
