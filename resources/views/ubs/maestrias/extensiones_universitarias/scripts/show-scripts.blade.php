<script type="module">
    ...
    $(document).ready(function () {
        var estado = {!!json_encode($extension->estado, JSON_HEX_TAG) !!};
        if (estado == 'IN' || estado == 'CO') {
            new Cleave('#horas', formatoSeparadorMiles);
        }
    ...
    function save(id, url) {
        ...
        $.ajax({
            ...
            var url_show = "{{route('extensiones_universitarias_ubs.show', ":id")}}";
    ...
    $('#cargar-horas-btn').click(function () {
        var nombre = $('#nombre_proyecto').val().toUpperCase();
        const swalWithBootstrapButtons = Swal.mixin({
            customClass: {
                confirmButton: 'btn btn-success me-2',
        ...
        swalWithBootstrapButtons.fire({
            title: '¿Está seguro de cargar las horas del alumno de la extensión universitaria ' + nombre + '?',
            icon: 'question',
            showCancelButton: true,
    ...
    $('#editar-horas-btn').click(function () {
        var nombre = $('#nombre_proyecto').val().toUpperCase();
        const swalWithBootstrapButtons = Swal.mixin({
            customClass: {
                confirmButton: 'btn btn-success me-2',
        ...
        swalWithBootstrapButtons.fire({
            title: '¿Está seguro de actualizar las horas del alumno de la extensión universitaria ' + nombre + '?',
            icon: 'question',
            showCancelButton: true,
    ...
    function update_hours(id, url) {
        const formData = new FormData(document.getElementById('change-horas-form'));
        $('#horas-form').find('.is-invalid').removeClass('is-invalid');
        $('#horas-form').find('.invalid-feedback').remove();
        var type = 'success';
        $.ajax({
            url: url,
            data: formData,
