@can('ver_actas_maestrias_ubs')
    ...
    @section('content')
        ...
        @include('ubs.maestrias.actas_evaluaciones.modals.index-modals')

        <div class="row">
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title mb-0">Lista de Actas de Evaluaciones de {{$curso->nombre_fantasia}}</h4>
                    </div>
                    <!-- end card header -->
                    <div class="card-body">
                            ...
                            <div class="row g-4 mb-3">
                                <div class="col-lg-8">
                                    @can('crear_actas_maestrias_ubs')
                                        <button type="button" class="btn btn-success" data-bs-toggle="modal" data-bs-target="#addActaModal"><i class="ri-add-line align-bottom me-1"></i>Agregar</button>
                                    @endcan
                                </div>
                                <div class="col-lg-4">
                                    ...
                                    <tbody class="list form-check-all">
                                        @foreach ($actas_evaluaciones as $acta_evaluacion)
                                            <tr>
                                                ...
                                                <td class="curso">{{$acta_evaluacion->curso->nombre_fantasia}}</td>
                                                <td class="modulo">{{$acta_evaluacion->modulo->nombre_fantasia}}</td>
                                                <td class="docente">{{$acta_evaluacion->docente->primer_nombre}} {{$acta_evaluacion->docente->primer_apellido}}</td>
                                                <td class="fecha">{{\Carbon\Carbon::parse($acta_evaluacion->fecha_evaluacion)->format('d/m/Y')}}</td>
                                                <td>
                                                    <a type="button" class="btn btn-sm btn-primary" href="{{route('actas_evaluaciones_ubs.show', $acta_evaluacion->id)}}" data-bs-toggle="tooltip" data-bs-trigger="hover" data-bs-placement="top" title="Ver"><i class="ri-eye-fill"></i></a>
    ...
    @section('script')
        ...
        @include('ubs.maestrias.actas_evaluaciones.scripts.index-scripts')
    @endsection
@endcan
