<script type="module">
    ...
    function fetch() {
        $.ajax({
            type: 'GET',
            url: '{{route('tesis_parametros_ubs.lineas_index_ajax')}}',
            success: function (response) {
                $('tbody').html('');
                $.each(response.lineas_tesis, function (index, value) {
                    if (value.estado == 'AC') {
                        ...
                        var botones = `<button class="btn btn-sm btn-primary" data-bs-toggle="modal" data-bs-target="#showModal" id="show-btn" data-id="${value.id}" data-bs-toggle="tooltip" data-bs-trigger="hover" data-bs-placement="top" title="Ver"><i class="ri-eye-fill"></i></button>
                                        ...
                                        @endcan
                                        @can('inactivar_lineas_tesis_ubs')
                ...
                var lineas_tesisList = new List('lineas_tesis-list', options);
                lineas_tesisList.on('updated', function(list) {
    ...
    $('#save-btn').click(function () {
        ...
        swalWithBootstrapButtons.fire({
            title: '¿Está seguro de guardar la nueva línea de tesis?',
            icon: 'question',
    ...
    $(document).on('click', '#show-btn', function() {
        ...
        var url = "{{route('tesis_parametros_ubs.lineas_show', ":id")}}"
        ...
        $.ajax({
            ...
            $('#nombre-show').val(response.linea_tesis.nombre);
            if (response.linea_tesis.estado == 'AC') {
                $('#estado-show').val('ACTIVO');
            } else if (response.linea_tesis.estado == 'IN') {
    ...
    $(document).on('click', '#edit-btn', function() {
        ...
        const id = $(this).data('id');
        var url = "{{route('tesis_parametros_ubs.lineas_edit', ":id")}}"
        url = url.replace(':id', id);
        $.ajax({
            type: 'GET',
            url: url,
            ...
            $('#nombre-edit').val(response.linea_tesis.nombre);
            $('.update-btn').val(response.linea_tesis.id);
        }).fail(function(response) {
    ...
    $(document).on('click', '#unactivate-btn', function() {
        const id = $(this).data('id');
        var url = "{{route('tesis_parametros_ubs.lineas_get_unactivate', ":id")}}"
        url = url.replace(':id', id);
        $.ajax({
            type: 'GET',
            url: url,
            ...
            $('.mensaje').html('Está seguro de inactivar la línea de tesis ' + response.linea_tesis.nombre + '?');
            $('.unactivate-btn').val(response.linea_tesis.id)
        }).fail(function(response) {
    ...
    $(document).on('click', '#activate-btn', function() {
        const id = $(this).data('id');
        var url = "{{route('tesis_parametros_ubs.lineas_get_activate', ":id")}}"
        url = url.replace(':id', id);
        $.ajax({
            type: 'GET',
            url: url,
            ...
            $('.mensaje').html('Está seguro de activar la línea de tesis ' + response.linea_tesis.nombre + '?');
            $('.activate-btn').val(response.linea_tesis.id)
        }).fail(function(response) {
    ...
    $(document).on('click', '#destroy-btn', function() {
        const id = $(this).data('id');
        var url = "{{route('tesis_parametros_ubs.lineas_get_destroy', ":id")}}"
        url = url.replace(':id', id);
        $.ajax({
            type: 'GET',
            url: url,
            ...
            $('.mensaje').html('Está seguro de eliminar la línea de tesis ' + response.linea_tesis.nombre + '?');
            $('.delete-btn').val(response.linea_tesis.id)
        }).fail(function(response) {
    ...
    $('.update-btn').click(function () {
        ...
        swalWithBootstrapButtons.fire({
            title: '¿Está seguro de actualizar la línea de tesis ' + nombre + '?',
            icon: 'question',
            showCancelButton: true,
    ...
    $('.delete-btn').click(function (e) {
        ...
        var url = "{{route('tesis_parametros_ubs.lineas_destroy', ":id")}}"
        url = url.replace(':id', id);
        var type = 'error';
        $.ajax({
            url: url,
    ...
    $('.unactivate-btn').click(function (e) {
        ...
        var url = "{{route('tesis_parametros_ubs.lineas_unactivate', ":id")}}"
        url = url.replace(':id', id);
        var type = 'success';
        $.ajax({
            url: url,
    ...
    $('.activate-btn').click(function (e) {
        ...
        var url = "{{route('tesis_parametros_ubs.lineas_activate', ":id")}}"
        url = url.replace(':id', id);
        var type = 'success';
        $.ajax({
            url: url,
    ...
    function save() {
        ...
        var type = 'success';
        $.ajax({
            url: '{{route('tesis_parametros_ubs.lineas_store')}}',
            data: formData,
            type: 'POST',
            headers: {
                'X-CSRF-TOKEN': '{{csrf_token()}}'
    ...
    function update(id) {
        ...
        var url = "{{route('tesis_parametros_ubs.lineas_update', ":id")}}"
        url = url.replace(':id', id);
        var type = 'success';
        $('#update-form').find('.is-invalid').removeClass('is-invalid');
        $('#update-form').find('.invalid-feedback').remove();
