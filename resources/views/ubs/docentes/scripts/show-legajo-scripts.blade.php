...
<script type="module">
    ...
    $('#tipo_legajo').on('change', function () {
        $('#institucion_educativa').selectpicker('destroy');
        $('#pais').selectpicker('destroy');
        if ($(this).val() != 7) {
            $('#titulo_obtenido').prop('disabled', false);
            $('#institucion_educativa').prop('disabled', false);
            $('#pais').prop('disabled', false);
            $('#span-titulo-obtenido').html('(*)');
            $('#span-institucion-educativa').html('(*)');
            $('#span-pais').html('(*)');
            $('#institucion_educativa').selectpicker('render');
            $('#pais').selectpicker('render');
        } else {
    ...
    $(document).on('click', '#subir-legajo-btn', function () {
        $('#store-docente-legajo-form').find('.is-invalid').removeClass('is-invalid');
        $('#store-docente-legajo-form').find('.invalid-feedback').remove();
    })

    $('#cancel-legajo-btn').click(function () {
        var nombre = ($('#docente').val()).toUpperCase();
        const swalWithBootstrapButtons = Swal.mixin({
            customClass: {
                confirmButton: 'btn btn-danger me-2',
                cancelButton: 'btn btn-light',
        ...
        swalWithBootstrapButtons.fire({
            title: '¿Está seguro de cancelar la subida del archivo al legajo del docente ' + nombre + '?',
            icon: 'warning',
            showCancelButton: true,
    ...
    $('#save-legajo-btn').click(function () {
        ...
        swalWithBootstrapButtons.fire({
            title: '¿Está seguro de guardar el archivo subido al legajo del docente ' + nombre + '?',
            icon: 'question',
            showCancelButton: true,
    ...
    function saveLegajo(id) {
        const formData = new FormData(document.getElementById('store-docente-legajo-form'));
        var url = "{{route('docentes_ubs.subir_legajo', ":id")}}"
    ...
    $('.delete-legajo-btn').on('click', function () {
        ...
        swalWithBootstrapButtons.fire({
            title: '¿Está seguro de eliminar el ' + tipo + ' del docente ' + nombre + '?',
            icon: 'warning',
            showCancelButton: true,
            confirmButtonText: 'Sí, eliminar!',
            cancelButtonText: 'Cancelar',
