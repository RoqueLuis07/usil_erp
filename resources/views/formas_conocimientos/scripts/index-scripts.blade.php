<script type="module">
    ...
    function fetch() {
        $.ajax({
            type: 'GET',
            url: '{{route('formas_conocimientos.index_ajax')}}',
            success: function (response) {
                $('tbody').html('');
                $.each(response.formas_conocimientos, function (index, value) {
                    $('tbody').append(
                        `<tr>
                ...
                var formas_conocimientosList = new List('formas_conocimientos-list', options);
                formas_conocimientosList.on('updated', function(list) {
    ...
    $('#save-btn').click(function () {
        ...
        swalWithBootstrapButtons.fire({
            title: '¿Está seguro de guardar la nueva forma de conocimiento?',
            icon: 'question',
    ...
    $(document).on('click', '#show-btn', function() {
        ...
        var url = "{{route('formas_conocimientos.show', ":id")}}"
        ...
        $.ajax({
            ...
            $('#nombre-show').val(response.forma_conocimiento.nombre);
    ...
    $(document).on('click', '#edit-btn', function() {
        ...
        const id = $(this).data('id');
        var url = "{{route('formas_conocimientos.edit', ":id")}}"
        url = url.replace(':id', id);
        $.ajax({
            type: 'GET',
            url: url,
            ...
            $('#nombre-edit').val(response.forma_conocimiento.nombre);
            $('.update-btn').val(response.forma_conocimiento.id);
        }).fail(function(response) {
    ...
    $(document).on('click', '#destroy-btn', function() {
        const id = $(this).data('id');
        var url = "{{route('formas_conocimientos.get_destroy', ":id")}}"
        url = url.replace(':id', id);
        $.ajax({
            type: 'GET',
            url: url,
            ...
            console.log(response.forma_conocimiento);
            $('.mensaje').html('Está seguro de eliminar la forma de conocimiento ' + response.forma_conocimiento.nombre + '?');
            $('.delete-btn').val(response.forma_conocimiento.id)
        }).fail(function(response) {
    ...
    $('.update-btn').click(function () {
        ...
        swalWithBootstrapButtons.fire({
            title: '¿Está seguro de actualizar la forma de conocimiento ' + nombre + '?',
            icon: 'question',
            showCancelButton: true,
    ...
    $('.delete-btn').click(function (e) {
        ...
        var url = "{{route('formas_conocimientos.destroy', ":id")}}"
        url = url.replace(':id', id);
        var type = 'error';
        $.ajax({
            url: url,
    ...
    function save() {
        ...
        var type = 'success';
        $.ajax({
            url: '{{route('formas_conocimientos.store')}}',
            data: formData,
            type: 'POST',
            headers: {
                'X-CSRF-TOKEN': '{{csrf_token()}}'
    ...
    function update(id) {
        ...
        var url = "{{route('formas_conocimientos.update', ":id")}}"
        url = url.replace(':id', id);
        var type = 'success';
        $('#update-form').find('.is-invalid').removeClass('is-invalid');
        $('#update-form').find('.invalid-feedback').remove();
