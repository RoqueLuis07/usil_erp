<script type="module">
    ...
    function fetch() {
        $.ajax({
            ...
            success: function (response) {
                ...
                var paisesList = new List('paises-list', options);
                paisesList.on('updated', function(list) {
    ...
    $(document).on('click', '#show-btn', function() {
        ...
        $.ajax({
            ...
            $('#nombre-show').val(response.pais.nombre);
    ...
    $(document).on('click', '#edit-btn', function() {
        ...
        $.ajax({
            ...
            $('#nombre-edit').val(response.pais.nombre);
            $('.update-btn').val(response.pais.id);
        }).fail(function(response) {
    ...
    $(document).on('click', '#destroy-btn', function() {
        const id = $(this).data('id');
        var url = "{{route('paises.get_destroy', ":id")}}"
        url = url.replace(':id', id);
        $.ajax({
            type: 'GET',
            url: url,
            ...
            $('.mensaje').html('Está seguro de eliminar el país ' + response.pais.nombre + '?');
            $('.delete-btn').val(response.pais.id)
        }).fail(function(response) {
    ...
    $('.update-btn').click(function () {
        ...
        swalWithBootstrapButtons.fire({
            title: '¿Está seguro de actualizar el país ' + nombre + '?',
            icon: 'question',
            showCancelButton: true,
    ...
    $('.delete-btn').click(function (e) {
        ...
        var url = "{{route('paises.destroy', ":id")}}"
        url = url.replace(':id', id);
        var type = 'error';
        $.ajax({
            url: url,
    ...
    function update(id) {
        ...
        var url = "{{route('paises.update', ":id")}}"
        url = url.replace(':id', id);
        var type = 'success';
        $('#update-form').find('.is-invalid').removeClass('is-invalid');
        $('#update-form').find('.invalid-feedback').remove();
