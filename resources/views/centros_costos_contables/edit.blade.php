@can('editar_centros_costos_contables')
    ...
    @section('title') Editar Centro de Costos @endsection
    @section('css')
    ...
    @section('content')
        @component('components.breadcrumb')
            ...
            @slot('title') Editar Centro de Costos  @endslot
        @endcomponent

        <div class="row">
            <form action="{{route('centros_costos_contables.update', $centro_costo->id)}}" method="post" id="update-form">
                @csrf
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-header">
                            <h4 class="card-title mb-0">Actualizar centro de costo</h4>
                        </div>
                        <!-- end card header -->
                        <div class="card-body">
                            ...
                            <div class="row">
                                <div class="col-lg-4 mb-3">
                                    ...
                                    <input type="text" class="form-control @error('nombre') is-invalid @enderror" id="nombre" name="nombre" value="{{old('nombre', $centro_costo->nombre)}}">
                                    @error('nombre')
                                    ...
                                    @foreach ($centro_costo->subcentrosCostosContables as $key => $detalle)
                                        <div class="mb-2 fila" id="fila-{{$key}}">
                                            <div class="row d-flex flex-wrap justify-content-center">
                                                <div class="col-lg-4 mb-2 text-center" id="div-nombre-{{$key}}">
                                                    @if ($key == 0) <label class="form-label label-nombre">Nombre <span class="text-danger">(*)</span></label> @endif
                                                    <input type="text" class="form-control text-center nombre-{{$key}} nombre @error('detalles.'. $key . '.nombre') is-invalid @enderror" id="detalles[{{$key}}][nombre]" name="detalles[{{$key}}][nombre]" value="{{old('detalles.' . $key . '.nombre', $detalle->nombre)}}" data-id="{{$key}}">
                                                    @error('detalles.'. $key . '.nombre')
                                                        <span class="invalid-feedback" role="alert">
                                                            <strong>{{$message}}</strong>
                                                        </span>
                                                    @enderror
    ...
    @section('script')
        ...
        @include('centros_costos_contables.scripts.edit-scripts')
        @include('centros_costos_contables.scripts.edit-detalles-scripts')
    @endsection
@endcan
