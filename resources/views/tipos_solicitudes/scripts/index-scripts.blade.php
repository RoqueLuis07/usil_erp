<script type="module">
    ...
    function fetch() {
        $.ajax({
            type: 'GET',
            url: '{{route('tipos_solicitudes.index_ajax')}}',
            success: function (response) {
                $('tbody').html('');
                $.each(response.tipos_solicitudes, function (index, value) {
                    if (value.estado == 'AC') {
                ...
                var tipos_solicitudesList = new List('tipos_solicitudes-list', options);
                tipos_solicitudesList.on('updated', function(list) {
    ...
    $('#save-btn').click(function () {
        ...
        swalWithBootstrapButtons.fire({
            title: '¿Está seguro de guardar el nuevo tipo de solicitud?',
            icon: 'question',
    ...
    $(document).on('click', '#show-btn', function() {
        ...
        var url = "{{route('tipos_solicitudes.show', ":id")}}"
        ...
        $.ajax({
            ...
            $('#nombre-show').val(response.tipo_solicitud.nombre);
            if (response.tipo_solicitud.pagado == true) {
                $('#pagado1-show').prop('checked', false);
                $('#pagado1-show').addClass('d-none');
                $('#label-pagado1-show').addClass('d-none');
                $('#pagado2-show').prop('checked', true);
                $('#pagado2-show').removeClass('d-none');
                $('#label-pagado2-show').removeClass('d-none');
            } else {
                $('#pagado2-show').prop('checked', false);
                $('#pagado2-show').addClass('d-none');
                $('#label-pagado2-show').addClass('d-none');
                $('#pagado1-show').prop('checked', true);
                $('#pagado1-show').removeClass('d-none');
                $('#label-pagado1-show').removeClass('d-none');
            }
            if (response.tipo_solicitud.estado == 'AC') {
                $('#estado-show').val('ACTIVO');
            } else if (response.tipo_solicitud.estado == 'IN') {
    ...
    $(document).on('click', '#edit-btn', function() {
        ...
        const id = $(this).data('id');
        var url = "{{route('tipos_solicitudes.edit', ":id")}}"
        url = url.replace(':id', id);
        $.ajax({
            type: 'GET',
            url: url,
            ...
            $('#nombre-edit').val(response.tipo_solicitud.nombre);
            if (response.tipo_solicitud.pagado == true) {
                $('#pagado2-edit').prop('checked', true);
            } else {
                $('#pagado1-edit').prop('checked', true);
            }
            $('.update-btn').val(response.tipo_solicitud.id);
        }).fail(function(response) {
    ...
    $(document).on('click', '#unactivate-btn', function() {
        const id = $(this).data('id');
        var url = "{{route('tipos_solicitudes.get_unactivate', ":id")}}"
        url = url.replace(':id', id);
        $.ajax({
            type: 'GET',
            url: url,
            ...
            $('.mensaje').html('Está seguro de inactivar el tipo de solicitud ' + response.tipo_solicitud.nombre + '?');
            $('.unactivate-btn').val(response.tipo_solicitud.id)
        }).fail(function(response) {
    ...
    $(document).on('click', '#activate-btn', function() {
        const id = $(this).data('id');
        var url = "{{route('tipos_solicitudes.get_activate', ":id")}}"
        url = url.replace(':id', id);
        $.ajax({
            type: 'GET',
            url: url,
            ...
            $('.mensaje').html('Está seguro de activar el tipo de solicitud ' + response.tipo_solicitud.nombre + '?');
            $('.activate-btn').val(response.tipo_solicitud.id)
        }).fail(function(response) {
    ...
    $(document).on('click', '#destroy-btn', function() {
        const id = $(this).data('id');
        var url = "{{route('tipos_solicitudes.get_destroy', ":id")}}"
        url = url.replace(':id', id);
        $.ajax({
            type: 'GET',
            url: url,
            ...
            $('.mensaje').html('Está seguro de eliminar el tipo de solicitud ' + response.tipo_solicitud.nombre + '?');
            $('.delete-btn').val(response.tipo_solicitud.id)
        }).fail(function(response) {
    ...
    $('.update-btn').click(function () {
        ...
        swalWithBootstrapButtons.fire({
            title: '¿Está seguro de actualizar el tipo de solicitud ' + nombre + '?',
            icon: 'question',
            showCancelButton: true,
    ...
    $('.delete-btn').click(function (e) {
        ...
        var url = "{{route('tipos_solicitudes.destroy', ":id")}}"
        url = url.replace(':id', id);
        var type = 'error';
        $.ajax({
            url: url,
    ...
    $('.unactivate-btn').click(function (e) {
        ...
        var url = "{{route('tipos_solicitudes.unactivate', ":id")}}"
        url = url.replace(':id', id);
        var type = 'success';
        $.ajax({
            url: url,
    ...
    $('.activate-btn').click(function (e) {
        ...
        var url = "{{route('tipos_solicitudes.activate', ":id")}}"
        url = url.replace(':id', id);
        var type = 'success';
        $.ajax({
            url: url,
    ...
    function save() {
        ...
        var type = 'success';
        $.ajax({
            url: '{{route('tipos_solicitudes.store')}}',
            data: formData,
            type: 'POST',
            headers: {
                'X-CSRF-TOKEN': '{{csrf_token()}}'
    ...
    function update(id) {
        ...
        var url = "{{route('tipos_solicitudes.update', ":id")}}"
        url = url.replace(':id', id);
        var type = 'success';
        $('#update-form').find('.is-invalid').removeClass('is-invalid');
        $('#update-form').find('.invalid-feedback').remove();
