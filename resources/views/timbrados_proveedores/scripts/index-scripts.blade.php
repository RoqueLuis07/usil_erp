<script type="module">
    ...
    var numero_edit = new Cleave('#numero-edit', formatoNumero);

    $(document).ready(function() {
        fetch();

        new Cleave('#numero', formatoNumero);
    });

    function fetch() {
        $.ajax({
            type: 'GET',
            url: '{{route('timbrados_proveedores.index_ajax')}}',
                ...
                $.each(response.timbrados, function (index, value) {
                    if (value.estado == 'AC') {
                        ...
                        var botones = `<button class="btn btn-sm btn-primary" data-bs-toggle="modal" data-bs-target="#showModal" id="show-btn" data-id="${value.id}" data-bs-toggle="tooltip" data-bs-trigger="hover" data-bs-placement="top" title="Ver"><i class="ri-eye-fill align-bottom"></i></button>
                                        @can('editar_timbrados_proveedores')
                                        ...
                                        @endcan
                                        @can('inactivar_timbrados_proveedores')
                                        ...
                                        @endcan
                                        @can('eliminar_timbrados_proveedores')
                        ...
                        var botones = `<button class="btn btn-sm btn-primary" data-bs-toggle="modal" data-bs-target="#showModal" id="show-btn" data-id="${value.id}" data-bs-toggle="tooltip" data-bs-trigger="hover" data-bs-placement="top" title="Ver"><i class="ri-eye-fill align-bottom"></i></button>
                                        @can('editar_timbrados_proveedores')
                                            <button class="btn btn-sm btn-info" data-bs-toggle="modal" data-bs-target="#editModal" id="edit-btn" data-id="${value.id}" data-bs-toggle="tooltip" data-bs-trigger="hover" data-bs-placement="top" title="Editar"><i class="ri-edit-fill align-bottom"></i></button>
                                        @endcan
                                        @can('activar_timbrados_proveedores')
                    ...
                    $('tbody').append(
                        `<tr>
                            <td>${value.id}</td>
                            <td class="proveedor">${value.proveedor.razon_social}</td>
                            <td class="numero">${value.numero}</td>
                            <td class="valido_desde">${value.valido_desde}</td>
                            <td class="valido_hasta">${value.valido_hasta}</td>
                            <td>${estado}</td>
                            <td>${botones}</td>`
                    )
                });
                var options = {
                    valueNames: ['proveedor', 'numero', 'valido_desde', 'valido_hasta'],
                    page: 50,
                    pagination: true
                };
                var timbradosList = new List('timbrados-list', options);
                timbradosList.on('updated', function(list) {
    ...
    $('#save-btn').click(function () {
        ...
        swalWithBootstrapButtons.fire({
            title: '¿Está seguro de guardar el nuevo timbrado de proveedor?',
            icon: 'question',
    ...
    $(document).on('click', '#show-btn', function() {
        ...
        var url = "{{route('timbrados_proveedores.show', ":id")}}"
        ...
        $.ajax({
            ...
            $('#proveedor-show').val(response.timbrado.proveedor.razon_social);
            $('#numero-show').val(response.timbrado.numero);
            $('#valido_desde-show').val(response.timbrado.valido_desde);
            $('#valido_hasta-show').val(response.timbrado.valido_hasta);
            if (response.timbrado.estado == 'AC') {
                $('#estado-show').val('ACTIVO');
            } else if (response.timbrado.estado == 'IN') {
    ...
    $(document).on('click', '#edit-btn', function() {
        ...
        const id = $(this).data('id');
        var url = "{{route('timbrados_proveedores.edit', ":id")}}"
        url = url.replace(':id', id);
        $.ajax({
            type: 'GET',
            url: url,
            ...
            $('#proveedor-edit').append('<option value="" disabled>Seleccionar...</option>');
            $.each(response.proveedores, function (index, value) {
                if (value.id == response.timbrado.proveedor_id) {
                    $('#proveedor-edit').append('<option value="' + value.id + '" selected>' + value.razon_social + '</option>');
                } else {
                    $('#proveedor-edit').append('<option value="' + value.id + '">' + value.razon_social + '</option>');
                }
            });
            $('#proveedor-edit').selectpicker('render');
            numero_edit.setRawValue(response.timbrado.numero);
            $('#valido_desde-edit').val(response.timbrado.valido_desde);
            $('#valido_hasta-edit').val(response.timbrado.valido_hasta);
            $('.update-btn').val(response.timbrado.id);
        }).fail(function(response) {
    ...
    $(document).on('click', '#unactivate-btn', function() {
        const id = $(this).data('id');
        var url = "{{route('timbrados_proveedores.get_unactivate', ":id")}}"
        url = url.replace(':id', id);
        $.ajax({
            type: 'GET',
            url: url,
            ...
            $('.mensaje').html('Está seguro de inactivar el timbrado ' + response.timbrado.numero + ' del proveedor ' + response.timbrado.proveedor.razon_social + '?');
            $('.unactivate-btn').val(response.timbrado.id)
        }).fail(function(response) {
    ...
    $(document).on('click', '#activate-btn', function() {
        const id = $(this).data('id');
        var url = "{{route('timbrados_proveedores.get_activate', ":id")}}"
        url = url.replace(':id', id);
        $.ajax({
            type: 'GET',
            url: url,
            ...
            $('.mensaje').html('Está seguro de activar el timbrado ' + response.timbrado.numero + ' del proveedor ' + response.timbrado.proveedor.razon_social + '?');
            $('.activate-btn').val(response.timbrado.id)
        }).fail(function(response) {
    ...
    $(document).on('click', '#destroy-btn', function() {
        const id = $(this).data('id');
        var url = "{{route('timbrados_proveedores.get_destroy', ":id")}}"
        url = url.replace(':id', id);
        $.ajax({
            type: 'GET',
            url: url,
            ...
            $('.mensaje').html('Está seguro de eliminar el timbrado ' + response.timbrado.numero + ' del proveedor ' + response.timbrado.proveedor.razon_social + '?');
            $('.delete-btn').val(response.timbrado.id)
        }).fail(function(response) {
    ...
    $('.update-btn').click(function () {
        var proveedor = $('#proveedor-edit option:selected').text();
        ...
        swalWithBootstrapButtons.fire({
            title: '¿Está seguro de actualizar el timbrado ' + numero + ' del proveedor ' + proveedor + '?',
            icon: 'question',
            showCancelButton: true,
    ...
    $('.delete-btn').click(function (e) {
        ...
        var url = "{{route('timbrados_proveedores.destroy', ":id")}}"
        url = url.replace(':id', id);
        var type = 'error';
        $.ajax({
            url: url,
    ...
    $('.unactivate-btn').click(function (e) {
        ...
        var url = "{{route('timbrados_proveedores.unactivate', ":id")}}"
        url = url.replace(':id', id);
        var type = 'success';
        $.ajax({
            url: url,
    ...
    $('.activate-btn').click(function (e) {
        ...
        var url = "{{route('timbrados_proveedores.activate', ":id")}}"
        url = url.replace(':id', id);
        var type = 'success';
        $.ajax({
            url: url,
    ...
    function save() {
        ...
        var type = 'success';
        $.ajax({
            url: '{{route('timbrados_proveedores.store')}}',
            data: formData,
            type: 'POST',
            headers: {
                'X-CSRF-TOKEN': '{{csrf_token()}}'
    ...
    function update(id) {
        ...
        var url = "{{route('timbrados_proveedores.update', ":id")}}"
        url = url.replace(':id', id);
        var type = 'success';
        $('#update-form').find('.is-invalid').removeClass('is-invalid');
        $('#update-form').find('.invalid-feedback').remove();
