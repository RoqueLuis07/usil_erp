<script type="module">
    ...
    function fetch() {
        $.ajax({
            type: 'GET',
            url: '{{route('tipos_movimientos.index_ajax')}}',
            success: function (response) {
                $('tbody').html('');
                $.each(response.tipos_movimientos, function (index, value) {
                    if (value.estado == 'AC') {
                        ...
                        var botones = `<button class="btn btn-sm btn-primary" data-bs-toggle="modal" data-bs-target="#showModal" id="show-btn" data-id="${value.id}" data-bs-toggle="tooltip" data-bs-trigger="hover" data-bs-placement="top" title="Ver"><i class="ri-eye-fill"></i></button>
                                        ...
                                        @endcan
                                        @can('inactivar_tipos_movimientos')
                                        ...
                                        @endcan
                                        @can('eliminar_tipos_movimientos')
                    ...
                    if (value.tipo == 'CA') {
                        var tipo = 'CAJAS';
                    } else if (value.tipo == 'BA') {
                        var tipo = 'BANCOS';
                    } else if (value.tipo == 'CB') {
                        var tipo = 'ENTRE CAJAS Y BANCOS';
                    } else if (value.tipo == 'DE') {
                        var tipo = 'DEPOSITOS';
                    }
                    $('tbody').append(
                        `<tr>
                ...
                var options = {
                    valueNames: ['nombre', 'tipo'],
                    page: 50,
                    pagination: true
                };
                var tipos_movimientosList = new List('tipos_movimientos-list', options);
                tipos_movimientosList.on('updated', function(list) {
    ...
    $('#save-btn').click(function () {
        ...
        swalWithBootstrapButtons.fire({
            title: '¿Está seguro de guardar el nuevo tipo de movimiento?',
            icon: 'question',
    ...
    $(document).on('click', '#show-btn', function() {
        ...
        var url = "{{route('tipos_movimientos.show', ":id")}}"
        ...
        $.ajax({
            ...
            $('#nombre-show').val(response.tipo_movimiento.nombre);
            if (value.tipo == 'CA') {
                var tipo = 'CAJAS';
            } else if (value.tipo == 'BA') {
                var tipo = 'BANCOS';
            ...
            $('#tipo-show').val(tipo);
            if (response.tipo_movimiento.estado == 'AC') {
                $('#estado-show').val('ACTIVO');
            } else if (response.tipo_movimiento.estado == 'IN') {
    ...
    $(document).on('click', '#edit-btn', function() {
        ...
        const id = $(this).data('id');
        var url = "{{route('tipos_movimientos.edit', ":id")}}"
        url = url.replace(':id', id);
        $.ajax({
            type: 'GET',
            url: url,
            ...
            $('#nombre-edit').val(response.tipo_movimiento.nombre);
            $('#tipo-edit').selectpicker('destroy');
            $('#tipo-edit').empty();
            $('#tipo-edit').append('<option value="" disabled>Seleccionar...</option>');
            if (response.tipo_movimiento.tipo == 'CA') {
                $('#tipo-edit').append('<option value="CA" selected>CAJAS</option>');
                $('#tipo-edit').append('<option value="BA">BANCOS</option>');
                $('#tipo-edit').append('<option value="CB">ENTRE CAJAS Y BANCOS</option>');
                $('#tipo-edit').append('<option value="DE">DEPOSITOS</option>');
            } else if (response.tipo_movimiento.tipo == 'BA') {
                $('#tipo-edit').append('<option value="CA">CAJAS</option>');
                $('#tipo-edit').append('<option value="BA" selected>BANCOS</option>');
                $('#tipo-edit').append('<option value="CB">ENTRE CAJAS Y BANCOS</option>');
                $('#tipo-edit').append('<option value="DE">DEPOSITOS</option>');
            } else if (response.tipo_movimiento.tipo == 'CB') {
                $('#tipo-edit').append('<option value="CA">CAJAS</option>');
                $('#tipo-edit').append('<option value="BA">BANCOS</option>');
                $('#tipo-edit').append('<option value="CB" selected>ENTRE CAJAS Y BANCOS</option>');
                $('#tipo-edit').append('<option value="DE">DEPOSITOS</option>');
            } else if (response.tipo_movimiento.tipo == 'DE') {
                $('#tipo-edit').append('<option value="CA">CAJAS</option>');
                $('#tipo-edit').append('<option value="BA">BANCOS</option>');
                $('#tipo-edit').append('<option value="CB">ENTRE CAJAS Y BANCOS</option>');
                $('#tipo-edit').append('<option value="DE" selected>DEPOSITOS</option>');
            }
            $('#tipo-edit').selectpicker('render');
            $('.update-btn').val(response.tipo_movimiento.id);
        }).fail(function(response) {
    ...
    $(document).on('click', '#unactivate-btn', function() {
        const id = $(this).data('id');
        var url = "{{route('tipos_movimientos.get_unactivate', ":id")}}"
        url = url.replace(':id', id);
        $.ajax({
            type: 'GET',
            url: url,
            ...
            $('.mensaje').html('Está seguro de inactivar el tipo de movimiento ' + response.tipo_movimiento.nombre + '?');
            $('.unactivate-btn').val(response.tipo_movimiento.id)
        }).fail(function(response) {
    ...
    $(document).on('click', '#activate-btn', function() {
        const id = $(this).data('id');
        var url = "{{route('tipos_movimientos.get_activate', ":id")}}"
        url = url.replace(':id', id);
        $.ajax({
            type: 'GET',
            url: url,
            ...
            $('.mensaje').html('Está seguro de activar el tipo de movimiento ' + response.tipo_movimiento.nombre + '?');
            $('.activate-btn').val(response.tipo_movimiento.id)
        }).fail(function(response) {
    ...
    $(document).on('click', '#destroy-btn', function() {
        const id = $(this).data('id');
        var url = "{{route('tipos_movimientos.get_destroy', ":id")}}"
        url = url.replace(':id', id);
        $.ajax({
            type: 'GET',
            url: url,
            ...
            $('.mensaje').html('Está seguro de eliminar el tipo de movimiento ' + response.tipo_movimiento.nombre + '?');
            $('.delete-btn').val(response.tipo_movimiento.id)
        }).fail(function(response) {
    ...
    $('.update-btn').click(function () {
        ...
        swalWithBootstrapButtons.fire({
            title: '¿Está seguro de actualizar el tipo de movimiento ' + nombre + '?',
            icon: 'question',
            showCancelButton: true,
    ...
    $('.delete-btn').click(function (e) {
        ...
        var url = "{{route('tipos_movimientos.destroy', ":id")}}"
        url = url.replace(':id', id);
        var type = 'error';
        $.ajax({
            url: url,
    ...
    $('.unactivate-btn').click(function (e) {
        ...
        var url = "{{route('tipos_movimientos.unactivate', ":id")}}"
        url = url.replace(':id', id);
        var type = 'success';
        $.ajax({
            url: url,
    ...
    $('.activate-btn').click(function (e) {
        ...
        var url = "{{route('tipos_movimientos.activate', ":id")}}"
        url = url.replace(':id', id);
        var type = 'success';
        $.ajax({
            url: url,
    ...
    function save() {
        ...
        var type = 'success';
        $.ajax({
            url: '{{route('tipos_movimientos.store')}}',
            data: formData,
            type: 'POST',
            headers: {
                'X-CSRF-TOKEN': '{{csrf_token()}}'
    ...
    function update(id) {
        ...
        var url = "{{route('tipos_movimientos.update', ":id")}}"
        url = url.replace(':id', id);
        var type = 'success';
        $('#update-form').find('.is-invalid').removeClass('is-invalid');
        $('#update-form').find('.invalid-feedback').remove();
