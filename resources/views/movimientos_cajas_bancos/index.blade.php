@can('ver_cajas_cuentas_movimientos')
    ...
    @section('title') Movimientos entre Cajas y Bancos @endsection
    @section('css')
    ...
    @section('content')
        @component('components.breadcrumb')
            ...
            @slot('title') Movimientos entre Cajas y Bancos @endslot
        @endcomponent

        @include('movimientos_cajas_bancos.scripts.messages-scripts')
        @include('movimientos_cajas_bancos.modals.index-modals')

        <div class="row">
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title mb-0">Lista de Movimientos entre Cajas y Bancos</h4>
                    </div>
                    <!-- end card header -->
                    <div class="card-body">
                        <div id="movimientos_cajas_bancos-list">
                            <div class="row g-4 mb-3">
                                <div class="col-lg-8">
                                    @can('crear_cajas_cuentas_movimientos')
                                        <a type="button" class="btn btn-success" href="{{route('movimientos_cajas_bancos.create')}}"><i class="ri-add-line align-bottom me-1"></i>Agregar</a>
                                    @endcan
                                </div>
                                <div class="col-lg-4">
                            ...
                            <div class="table-responsive table-card mt-3 mb-1">
                                <table class="table align-middle table-nowrap" id="movimientos_cajas_bancos-list">
                                    <thead class="table-light">
                                        <tr>
                                            ...
                                            <th></th>
                                            <th class="sort" data-sort="cuenta_bancaria">Cuenta Bancaria</th>
                                    ...
                                    <tbody class="list form-check-all">
                                        @foreach ($movimientos_cajas_bancos as $movimiento)
                                            <tr>
                                                ...
                                                <td class="cuenta_bancaria">{{$movimiento->cuentaBancaria->banco->nombre}} <span class="text-muted">{{$movimiento->cuentaBancaria->numero_cuenta}}</span></td>
                                                <td class="fecha">{{Carbon\Carbon::parse($movimiento->fecha)->format('d/m/Y H:i:s')}}</td>
                                                <td>
                                                    @if ($movimiento->tipo_movimiento_id == 7)
                                                        {{number_format($movimiento->monto, 0, ',', '.')}} GS
                                                    @elseif ($movimiento->tipo_movimiento_id == 8)
                                                        @if ($movimiento->cuentaBancaria->moneda_id == 1)
                                                            {{number_format($movimiento->monto, 0, ',', '.')}} {{$movimiento->cuentaBancaria->moneda->codigo}}
                                                        @else
                                                            {{number_format($movimiento->monto, 2, ',', '.')}} {{$movimiento->cuentaBancaria->moneda->codigo}}
                                                        @endif
                                                    @endif
                                                </td>
                                                <td class="estado">
                                                ...
                                                <td>
                                                    ...
                                                    @if ($movimiento->estado == 'PE')
                                                        @can('editar_cajas_cuentas_movimientos')
                                                            <a type="button" class="btn btn-sm btn-info" href="{{route('movimientos_cajas_bancos.edit', $movimiento->id)}}" data-bs-toggle="tooltip" data-bs-trigger="hover" data-bs-placement="top" title="Editar"><i class="ri-edit-fill"></i></a>
                                                        @endcan
                                                        @can('aprobar_cajas_cuentas_movimientos')
                                                        ...
                                                        @endcan
                                                        @can('rechazar_cajas_cuentas_movimientos')
                                                        ...
                                                        @endcan
                                                    @elseif ($movimiento->estado == 'RE')
                                                        @can('anular_rechazo_cajas_cuentas_movimientos')
                                                        ...
                                                        @endcan
                                                    @elseif ($movimiento->estado == 'AP')
                                                        @can('anular_aprobacion_cajas_cuentas_movimientos')
                                                        ...
                                                        @endcan
                                                    @endif
                                                    @can('eliminar_cajas_cuentas_movimientos')
    ...
    @section('script')
        ...
        @include('movimientos_cajas_bancos.scripts.index-scripts')
    @endsection
@endcan
