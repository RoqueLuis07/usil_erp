<script type="module">
    ...
    $(document).on('click', '.btn-erase', function () {
        ...
        detalle_accion_add.appendTo('#acciones-' + nro_ultima_fila);
    ...
    function insertarLabels(primera_fila) {
        ...
        if ($('.label-punto').text() == '') {
            $('<label class="form-label label-punto">Puntos <span class="text-danger">(*)</span></label>').insertBefore('.puntos-' + primera_fila);
        }
        ...
        if (errors) {
            ...
            $.each(arrayFilas, function (index) {
                ...
                if (index != 0) {
                    ...
                    $('.puntos-' + index).val(arrayFilas[index].punto);
                }
                calcularTotalPuntos();
            })
        }
