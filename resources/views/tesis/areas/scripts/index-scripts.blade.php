<script type="module">
    ...
    function fetch() {
        $.ajax({
            type: 'GET',
            url: '{{route('areas_tesis.index_ajax')}}',
            success: function (response) {
                $('tbody').html('');
                $.each(response.areas_tesis, function (index, value) {
                    if (value.estado == 'AC') {
                ...
                var areas_tesisList = new List('areas_tesis-list', options);
                areas_tesisList.on('updated', function(list) {
    ...
    $('#save-btn').click(function () {
        ...
        swalWithBootstrapButtons.fire({
            title: '¿Está seguro de guardar el nuevo área de trabajo final de grado?',
            icon: 'question',
    ...
    $(document).on('click', '#show-btn', function() {
        ...
        var url = "{{route('areas_tesis.show', ":id")}}"
        ...
        $.ajax({
            ...
            $('#nombre-show').val(response.area_tesis.nombre);
            if (response.area_tesis.estado == 'AC') {
                $('#estado-show').val('ACTIVO');
            } else if (response.area_tesis.estado == 'IN') {
    ...
    $(document).on('click', '#edit-btn', function() {
        ...
        const id = $(this).data('id');
        var url = "{{route('areas_tesis.edit', ":id")}}"
        url = url.replace(':id', id);
        $.ajax({
            type: 'GET',
            url: url,
            ...
            $('#nombre-edit').val(response.area_tesis.nombre);
            $('.update-btn').val(response.area_tesis.id);
        }).fail(function(response) {
    ...
    $(document).on('click', '#unactivate-btn', function() {
        const id = $(this).data('id');
        var url = "{{route('areas_tesis.get_unactivate', ":id")}}"
        url = url.replace(':id', id);
        $.ajax({
            type: 'GET',
            url: url,
            ...
            $('.mensaje').html('Está seguro de inactivar el área de trabajo final de grado ' + response.area_tesis.nombre + '?');
            $('.unactivate-btn').val(response.area_tesis.id)
        }).fail(function(response) {
    ...
    $(document).on('click', '#activate-btn', function() {
        const id = $(this).data('id');
        var url = "{{route('areas_tesis.get_activate', ":id")}}"
        url = url.replace(':id', id);
        $.ajax({
            type: 'GET',
            url: url,
            ...
            $('.mensaje').html('Está seguro de activar el área de trabajo final de grado ' + response.area_tesis.nombre + '?');
            $('.activate-btn').val(response.area_tesis.id)
        }).fail(function(response) {
    ...
    $(document).on('click', '#destroy-btn', function() {
        const id = $(this).data('id');
        var url = "{{route('areas_tesis.get_destroy', ":id")}}"
        url = url.replace(':id', id);
        $.ajax({
            type: 'GET',
            url: url,
            ...
            $('.mensaje').html('Está seguro de eliminar el área de trabajo final de grado ' + response.area_tesis.nombre + '?');
            $('.delete-btn').val(response.area_tesis.id)
        }).fail(function(response) {
    ...
    $('.update-btn').click(function () {
        ...
        swalWithBootstrapButtons.fire({
            title: '¿Está seguro de actualizar el área de trabajo final de grado ' + nombre + '?',
            icon: 'question',
            showCancelButton: true,
    ...
    $('.delete-btn').click(function (e) {
        ...
        var url = "{{route('areas_tesis.destroy', ":id")}}"
        url = url.replace(':id', id);
        var type = 'error';
        $.ajax({
            url: url,
    ...
    $('.unactivate-btn').click(function (e) {
        ...
        var url = "{{route('areas_tesis.unactivate', ":id")}}"
        url = url.replace(':id', id);
        var type = 'success';
        $.ajax({
            url: url,
    ...
    $('.activate-btn').click(function (e) {
        ...
        var url = "{{route('areas_tesis.activate', ":id")}}"
        url = url.replace(':id', id);
        var type = 'success';
        $.ajax({
            url: url,
    ...
    function update(id) {
        ...
        var url = "{{route('areas_tesis.update', ":id")}}"
        url = url.replace(':id', id);
        var type = 'success';
        $('#update-form').find('.is-invalid').removeClass('is-invalid');
        $('#update-form').find('.invalid-feedback').remove();
