<script type="module">
    ...
    $(document).on('click', '.btn-add', function () {
        ...
        if ($('.alumno-' + nro_ultima_fila + ' option:selected').val() != '' && $('.carreraa-' + nro_ultima_fila).val() != '') {
            var filaAdd = `<div class="mb-2 fila" id="fila-${nro_nueva_fila}">
                                <div class="row d-flex flex-wrap justify-content-center">
                                    <div class="col-lg-4 mb-2 text-center" id="div-alumno-${nro_nueva_fila}">
                                        <select class="selectpicker form-control alumno-${nro_nueva_fila} alumno @error('detalles.${nro_nueva_fila}.alumno') is-invalid @enderror" id="alumno-${nro_nueva_fila}" name="detalles[${nro_nueva_fila}][alumno]" data-live-search="true" data-live-search-normalize="true" data-id="${nro_nueva_fila}">
                                    ...
                                    <div class="col-lg-3 mb-2 me-3 text-center" id="div-semestre-${nro_nueva_fila}">
                                        <input type="hidden" class="carrera carrera-${nro_nueva_fila}" name="detalles[${nro_nueva_fila}][carrera]" value="{{old('detalles.${nro_nueva_fila}.carrera')}}">
                                        <input type="text" class="form-control text-center carrera_nombre-${nro_nueva_fila} carrera_nombre @error('detalles.${nro_nueva_fila}.carrera_nombre') is-invalid @enderror" id="detalles[${nro_nueva_fila}][carrera_nombre]" id="detalles[${nro_nueva_fila}][carrera_nombre]" value="{{old('detalles.${nro_nueva_fila}.carrera_nombre')}}" data-id="${nro_nueva_fila}" readonly>
                                        <span class="invalid-feedback error-carrera-${nro_nueva_fila}" role="alert">
    ...
    function insertarLabels(primera_fila) {
        ...
        if ($('.label-carrera').text() == '') {
            $('<label class="form-label label-carrera">Carrera <span class="text-danger">(*)</span></label>').insertBefore('.carrera-' + primera_fila);
        }
    ...
    function get_carrera(alumno, fila) {
        var url = "{{route('tutorias.get_carrera', ":alumno")}}"
        ...
        $.ajax({
            ...
            $('.carrera-' + fila).val(response.carrera.id);
            $('.carrera_nombre-' + fila).val(response.carrera.nombre_fantasia);
        })
    }
        ...
        if (errors) {
            ...
            $.each(arrayFilas, function (index) {
                ...
                if (index != 0 && index > arrayFilasLength) {
                    ...
                    $('.alumno-' + i1ndex).selectpicker('val', arrayFilas[index].alumno);
                    $('.carrera-' + index).val(arrayFilas[index].carrera);
                    $('.carrera_nombre-' + index).val(arrayFilas[index].carrera_nombre);
                }
