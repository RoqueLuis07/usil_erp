...
class ConvalidacionInternaController extends Controller
{
    /**
    ...
    {
        $this->authorize('ver_convalidaciones_internas');

        try {
            $convalidaciones = Convalidacion::where('tipo', 'IN')->orderBy('id', 'asc')->get();
            return view('convalidaciones/internas/index')->with(compact('convalidaciones'));
        } catch (\Exception $e) {
    ...
    public function show($id)
    {
        ...
        try {
            ...
            return view('convalidaciones/internas/show')->with(compact('convalidacion', 'materias', 'fecha_hoy'));
        } catch (\Exception $e) {
            return redirect()->route('convalidaciones_internas.index')->with('error-message', $e->getMessage());
        }
    }
    ...
    {
        $this->authorize('crear_convalidaciones_internas');
        ...
        try {
            ...
            return view('convalidaciones/internas/create')->with(compact('alumnos', 'carreras', 'usil', 'solicitud_nueva'));
        } catch (\Exception $e) {
            return redirect()->route('convalidaciones_internas.index')->with('error-message', $e->getMessage());
        }
    }
    ...
    public function store(Request $request)
    {
        ...
        //     return redirect()->route('convalidaciones_internas.index')->with('error-message', 'El alumno debe estar al día con sus pagos antes de realizar la solicitud de convalidación.');
        // }
        ...
        $request->validate([
            ...
            'carrera_origen' => ['required', 'numeric'],
        ...
        try {
            ...
            $convalidacion->tipo = 'IN';
            $convalidacion->alumno_id = $request->alumno;
            $convalidacion->universidad_origen_id = $request->universidad_origen;
            $carrera_origen = Carrera::findOrFail($request->carrera_origen);
            $convalidacion->carrera_origen = removeAccents(Str::upper($carrera_origen->nombre_fantasia));
            ...
            $convalidacion->facultad_id = $matriculacion->carrera->facultad_id;
            $convalidacion->programa_id = $matriculacion->carrera->programa_id;
            ...
            foreach ($request->detalles as $detalle) {
                ...
                $materia_origen = Materia::findOrFail($detalle['materia_origen']);
                $convalidacion_detalle->materia_origen = $materia_origen->nombre_fantasia;
                $convalidacion_detalle->carga_horaria_materia_origen = $materia_origen->carga_horaria;
            ...
            return redirect()->route('convalidaciones_internas.index')->with('success-message', 'La solicitud de convalidación N° ' . $convalidacion->numero_solicitud . ' del alumno ' . $convalidacion->alumno->primer_nombre . ' ' . $convalidacion->alumno->primer_apellido . ' fue creada exitosamente.');
        } catch (\Exception $e) {
            DB::rollback();
            return redirect()->route('convalidaciones_internas.index')->with('error-message', $e->getMessage());
        }
    ...
    {
        $this->authorize('editar_convalidaciones_internas');
        ...
        try {
            ...
            $carrera = Carrera::where('nombre_fantasia', $convalidacion->carrera_origen)->first();
            $malla = Malla::where('carrera_id', $carrera->id)->first();
            $materias = MallaDetalle::join('materias', 'mallas_detalles.materia_id', '=', 'materias.id')
            ...
            return view('convalidaciones/internas/edit')->with(compact('convalidacion', 'carreras', 'instituciones_educativas', 'usil', 'materias'));
        } catch (\Exception $e) {
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            $convalidacion = Convalidacion::findOrFail($id);
            $convalidacion->facultad_origen = Str::upper($request->facultad_origen);
            $convalidacion->carrera_origen = Str::upper($request->carrera_origen);
            $convalidacion->universidad_origen_id = $request->universidad_origen;
    ...
    {
        $this->authorize('dictaminar_convalidaciones_internas');

        $fecha_hoy = Carbon::now()->format('Y-m-d');

        $request->validate([
            'numero_dictamen' => ['required', Rule::unique('convalidaciones_detalles')->ignore($id)],
        ...
        try {
            ...
            $detalle->numero_dictamen = Str::upper($request->numero_dictamen);
            ...
            if ($request->dictamen) {
                ...
                $directorio = 'storage/convalidaciones/internas/dictamenes';
                if ($extension == 'jpg' || $extension == 'jpeg' || $extension == 'png') {
                    ...
                    $directorio_storage = 'public/convalidaciones/internas/dictamenes';
                    $nombre = Str::lower($detalle->numero_dictamen) . '_' . $detalle->fecha_dictamen;
                    $nombre = str_replace('/', '-', $nombre);
                    $nombre_archivo = $nombre . '.' . $extension;
    ...
    {
        $this->authorize('anular_dictamen_convalidaciones_internas');
        ...
        try {
            ...
            return redirect()->route('convalidaciones_internas.show', $convalidacion->id)->with('success-message', 'El dictaminado de la solicitud de convalidación N° ' . $convalidacion->numero_solicitud . ' fue revertida exitosamente.');
        } catch (\Exception $e) {
            DB::rollback();
            return redirect()->route('convalidaciones_internas.index')->with('error-message', $e->getMessage());
        }
    ...
    {
        $this->authorize('eliminar_adjunto_dictamen_convalidaciones_internas');
    ...
    {
        $this->authorize('resolucion_convalidaciones_internas');

        $fecha_hoy = Carbon::now()->format('Y-m-d');

        $request->validate([
            'numero_resolucion' => ['required', Rule::unique('convalidaciones_detalles')->ignore($id)],
        ...
        try {
            $detalle = ConvalidacionDetalle::findOrFail($id);
            $detalle->numero_resolucion = Str::upper($request->numero_resolucion);
            $detalle->fecha_resolucion = $request->fecha_resolucion;
            //cargar archivo
            if ($request->resolucion) {
                $archivo = $request->resolucion;
                $extension = $archivo->getClientOriginalExtension();
                $directorio = 'storage/convalidaciones/internas/resoluciones';
                if ($extension == 'jpg' || $extension == 'jpeg' || $extension == 'png') {
                    $manager = new ImageManager(Driver::class);
                    $image = $manager->read($archivo);
                    $image = $image->encode(new AutoEncoder(quality: 50));
                    ...
                    $directorio_storage = 'public/convalidaciones/internas/resoluciones';
                    $nombre = Str::lower($detalle->numero_resolucion) . '_' . $detalle->fecha_resolucion;
                    $nombre = str_replace('/', '-', $nombre);
                    $nombre_archivo = $nombre . '.' . $extension;
                    Storage::putFileAs($directorio_storage . '/', $archivo, $nombre_archivo);
    ...
    {
        $this->authorize('anular_resolucion_convalidaciones_internas');
        ...
        try {
            ...
            return redirect()->route('convalidaciones_internas.show', $convalidacion->id)->with('success-message', 'La aprobación de la solicitud de convalidación N° ' . $convalidacion->numero_solicitud . ' fue revertida exitosamente.');
        } catch (\Exception $e) {
            DB::rollback();
            return redirect()->route('convalidaciones_internas.index')->with('error-message', $e->getMessage());
        }
    ...
    {
        $this->authorize('eliminar_adjunto_resolucion_convalidaciones_internas');
    ...
    {
        $this->authorize('convalidar_convalidaciones_internas');
        ...
        try {
            ...
            foreach ($detalles as $detalle) {
                $inscripcion = Inscripcion::where('matriculacion_id', $matriculacion->id)->where('materia_id', $detalle->materia_id)->get();
                if ($inscripcion) {
                    if ($inscripcion->estado == 'MA' || $inscripcion->estado == 'EC') {
                        ...
                        return redirect()->route('convalidaciones_internas.index')->with('error-message', 'La convalidación no se puede realizar. El alumno ' . $convalidacion->alumno->primer_nombre . ' ' . $convalidacion->alumno->primer_apellido . '  ya cuenta con las materias seleccionadas aprobadas.');
                    }
                } else {
            ...
            return redirect()->route('convalidaciones_internas.index')->with('success-message', 'La convalidación de materias del alumno ' . $convalidacion->alumno->primer_nombre . ' ' . $convalidacion->alumno->primer_apellido . ' fue realizada existosamente.');
        } catch (\Exception $e) {
            DB::rollback();
            return redirect()->route('convalidaciones_internas.index')->with('error-message', $e->getMessage());
        }
    ...
    {
        $this->authorize('eliminar_convalidaciones_internas');
        ...
        try {
            ...
            if ($convalidacion->estado == 'SO') {
                ...
                return redirect()->route('convalidaciones_internas.index')->with('success-message','La solicitud de convalidación N° ' . $convalidacion->numero_solicitud . ' fue eliminada exitosamente.');
            } else {
                return redirect()->route('convalidaciones_internas.index')->with('error-message','La solicitud de convalidación N° ' . $convalidacion->numero_solicitud . ' no se puede eliminar. Ésta ya se encuentra dictaminada o aprobada.');
            }
        } catch (\Exception $e) {
            DB::rollback();
            ...
            if (stripos($e->getMessage(), 'Foreign key violation')) {
                return redirect()->route('convalidaciones_internas.index')->with('error-message', 'La solicitud de convalidación N° ' . $convalidacion->numero_solicitud . ' no se puede eliminar. Está siendo utilizado por otro registro dentro del sistema.');
            } else {
                return redirect()->route('convalidaciones_internas.index')->with('error-message', $e->getMessage());
            }
        }
    ...
    public function get_materias($carrera)
    {
        ...
        try {
            $malla = Malla::where('carrera_id', $carrera)->first();
    ...
    public function get_nota($alumno, $materia)
    {
        DB::beginTransaction();

        try {
            $alumno_nota = AlumnoNota::where('alumno_id', $alumno)->where('materia_id', $materia)->first();
            if (!$alumno_nota) {
                $nota = 'N/A';
            } else {
                $nota = $alumno_nota->calificacion;
            }
